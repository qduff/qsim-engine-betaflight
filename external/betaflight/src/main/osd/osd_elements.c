...
/*
    ...
    *****************************************
    ...
    Next add the element to the osdElementDisplayOrder array defined in this
   file. If the element needs special runtime conditional processing then it
   should be added to the osdAnalyzeActiveElements() function instead.

    Create the function to "draw" the element. It should be named like
   "osdElementSomething()" where the "Something" describes the element.

    Finally add the mapping from the element ID added in the first step to the
   function created in the third step to the osdElementDrawFunction array.
*/
...
#define OSD_STICK_OVERLAY_VERTICAL_POSITIONS \
...
static int getEscRpm(int i) {
    ...
    if (motorConfig()->dev.useDshotTelemetry) {
        return 100.0f / (motorConfig()->motorPoleCount / 2.0f) *
               getDshotTelemetry(i);
    }
#endif
...
static void renderOsdEscRpmOrFreq(getEscRpmOrFreqFnPtr escFnPtr,
...
static void osdFormatAltitudeString(char *buff, int32_t altitudeCm) {
    ...
    tfp_sprintf(
      buff, "%c%5d %c", SYM_ALTITUDE, alt, osdGetMetersToSelectedUnitSymbol());
    buff[6] = buff[5];
    buff[5] = '.';
}
...
static void osdFormatCoordinate(char *buff, char sym, int32_t val) {
    ...
    // eg: s-180.1234567z   s=symbol, z=zero terminator, decimal separator
    // between 0 and 1

    static const int coordinateMaxLength =
    ...
    const int written =
      tfp_sprintf(buff + 1, "%d.%07d", integerPart, decimalPart);
...
void osdFormatTimer(char *buff,
    ...
    osdFormatTime(
      buff,
      (usePrecision ? OSD_TIMER_PRECISION(timer) : OSD_TIMER_PREC_SECOND),
      osdGetTimerValue(src));
}

static char osdGetBatterySymbol(int cellVoltage) {
    if (getBatteryState() == BATTERY_CRITICAL) {
        return SYM_MAIN_BATT;  // FIXME: currently the BAT- symbol, ideally
                               // replace with a battery with exclamation mark
    } else {
        // Calculate a symbol offset using cell voltage over full cell voltage
        // range
        const int symOffset = scaleRange(cellVoltage,
...
static uint8_t osdGetHeadingIntoDiscreteDirections(int heading,
                                                   unsigned directions) {
    ...
    int direction = (heading * directions + FULL_CIRCLE / 2) /
                    FULL_CIRCLE;  // scale with rounding
...
static void osdElementAngleRollPitch(osdElementParms_t *element) {
    const int angle = (element->item == OSD_PITCH_ANGLE) ? attitude.values.pitch
                                                         : attitude.values.roll;
    tfp_sprintf(element->buff,
                "%c%c%02d.%01d",
                (element->item == OSD_PITCH_ANGLE) ? SYM_PITCH : SYM_ROLL,
                angle < 0 ? '-' : ' ',
                abs(angle / 10),
                abs(angle % 10));
}
...
static void osdElementArtificialHorizon(osdElementParms_t *element) {
    ...
    const int rollAngle =
      constrain(attitude.values.roll * ahSign, -maxRoll, maxRoll);
    int pitchAngle =
      constrain(attitude.values.pitch * ahSign, -maxPitch, maxPitch);
    // Convert pitchAngle to y compensation value
    // (maxPitch / 25) divisor matches previous settings of fixed divisor of 8
    // and fixed max AHI pitch angle of 20.0 degrees
    if (maxPitch > 0) {
    ...
    for (int x = -4; x <= 4; x++) {
        ...
        if (y >= 0 && y <= 81) {
            displayWriteChar(element->osdDisplayPort,
                             element->elemPosX + x,
                             element->elemPosY + (y / AH_SYMBOL_COUNT),
...
static void osdElementCoreTemperature(osdElementParms_t *element) {
    tfp_sprintf(
      ...
      "C%c%3d%c",
      SYM_TEMPERATURE,
      osdConvertTemperatureToSelectedUnit(getCoreTemperatureCelsius()),
      osdGetTemperatureSymbolForSelectedUnit());
}
#endif  // USE_ADC_INTERNAL

static void osdElementCraftName(osdElementParms_t *element) {
    // This does not strictly support iterative updating if the craft name
    // changes at run time. But since the craft name is not supposed to be
    // changing this should not matter, and blanking the entire length of the
    // craft name string on update will make it impossible to configure elements
    // to be displayed on the right hand side of the craft name.
    // TODO: When iterative updating is implemented, change this so the craft
    // name is only printed once whenever the OSD 'flight' screen is entered.
    ...
    if (strlen(pilotConfig()->name) == 0) {
        ...
        for (i = 0; i < MAX_NAME_LENGTH; i++) {
            if (pilotConfig()->name[i]) {
                element->buff[i] =
                  toupper((unsigned char)pilotConfig()->name[i]);
            } else {
                break;
            }
        }
...
static void osdElementCrashFlipArrow(osdElementParms_t *element) {
    ...
    if ((isFlipOverAfterCrashActive() ||
         (!ARMING_FLAG(ARMED) && !STATE(SMALL_ANGLE))) &&
        !((imuConfig()->small_angle < 180) && STATE(SMALL_ANGLE)) &&
        (rollAngle || pitchAngle)) {
        if (abs(pitchAngle) < 2 * abs(rollAngle) &&
            abs(rollAngle) < 2 * abs(pitchAngle)) {
            if (pitchAngle > 0) {
                if (rollAngle > 0) {
...
static void osdElementDisplayName(osdElementParms_t *element) {
    // This does not strictly support iterative updating if the display name
    // changes at run time. But since the display name is not supposed to be
    // changing this should not matter, and blanking the entire length of the
    // display name string on update will make it impossible to configure
    // elements to be displayed on the right hand side of the display name.
    // TODO: When iterative updating is implemented, change this so the display
    // name is only printed once whenever the OSD 'flight' screen is entered.
    ...
    if (strlen(pilotConfig()->displayName) == 0) {
        ...
        for (i = 0; i < MAX_NAME_LENGTH; i++) {
            if (pilotConfig()->displayName[i]) {
                element->buff[i] =
                  toupper((unsigned char)pilotConfig()->displayName[i]);
            } else {
                break;
            }
        }
...
static void osdElementRateProfileName(osdElementParms_t *element) {
    if (strlen(currentControlRateProfile->profileName) == 0) {
        tfp_sprintf(
          element->buff, "RATE_%u", getCurrentControlRateProfileIndex() + 1);
    } else {
        unsigned i;
        for (i = 0; i < MAX_PROFILE_NAME_LENGTH; i++) {
            if (currentControlRateProfile->profileName[i]) {
                element->buff[i] = toupper(
                  (unsigned char)currentControlRateProfile->profileName[i]);
            } else {
                break;
            }
        }
    ...
    if (strlen(currentPidProfile->profileName) == 0) {
        ...
        for (i = 0; i < MAX_PROFILE_NAME_LENGTH; i++) {
            if (currentPidProfile->profileName[i]) {
                element->buff[i] =
                  toupper((unsigned char)currentPidProfile->profileName[i]);
            } else {
                break;
            }
        }
...
static void osdElementEscTemperature(osdElementParms_t *element) {
    if (featureIsEnabled(FEATURE_ESC_SENSOR)) {
        tfp_sprintf(
          ...
          "E%c%3d%c",
          SYM_TEMPERATURE,
          osdConvertTemperatureToSelectedUnit(osdEscDataCombined->temperature),
          osdGetTemperatureSymbolForSelectedUnit());
    }
}
...
static void osdElementGpsFlightDistance(osdElementParms_t *element) {
    if (STATE(GPS_FIX) && STATE(GPS_FIX_HOME)) {
        const int32_t distance =
          osdGetMetersToSelectedUnit(GPS_distanceFlownInCm / 100);
        tfp_sprintf(
          element->buff, "%d%c", distance, osdGetMetersToSelectedUnitSymbol());
    } else {
...
static void osdElementGpsHomeDirection(osdElementParms_t *element) {
    if (STATE(GPS_FIX) && STATE(GPS_FIX_HOME)) {
        if (GPS_distanceToHome > 0) {
            const int h =
              GPS_directionToHome - DECIDEGREES_TO_DEGREES(attitude.values.yaw);
...
static void osdElementGpsSpeed(osdElementParms_t *element) {
    tfp_sprintf(element->buff,
                "%3d%c",
                osdGetSpeedToSelectedUnit(gpsSol.groundSpeed),
...
static void osdElementLinkQuality(osdElementParms_t *element) {
    // change range to 0-9 (two sig. fig. adds little extra value, also reduces
    // screen estate)
...
static void osdElementLogStatus(osdElementParms_t *element) {
    if (IS_RC_MODE_ACTIVE(BOXBLACKBOX)) {
        if (!isBlackboxDeviceWorking()) {
            ...
            tfp_sprintf(
              element->buff, "%c%d", SYM_BBLOG, blackboxGetLogNumber());
        }
    }
}
#endif  // USE_BLACKBOX
...
#define MAIN_BATT_USAGE_STEPS \
    11  // Use an odd number so the bar can be centered.

    // Calculate constrained value
    const float value =
      constrain(batteryConfig()->batteryCapacity - getMAhDrawn(),
                0,
                batteryConfig()->batteryCapacity);

    // Calculate mAh used progress
    const uint8_t mAhUsedProgress = ceilf(
      (value / (batteryConfig()->batteryCapacity / MAIN_BATT_USAGE_STEPS)));
...
static void osdElementMotorDiagnostics(osdElementParms_t *element) {
    ...
    for (; i < getMotorCount(); i++) {
        if (motorsRunning) {
            element->buff[i] =
              0x88 -
              scaleRange(motor[i], motorOutputLow, motorOutputHigh, 0, 8);
        } else {
            element->buff[i] = 0x88;
        }
    }
...
static void osdElementNumericalHeading(osdElementParms_t *element) {
    ...
    tfp_sprintf(element->buff,
                "%c%03d",
                osdGetDirectionSymbolFromHeading(heading),
                heading);
}
...
#endif  // USE_GPS
    if (haveBaro || haveGps) {
        const int verticalSpeed =
          osdGetMetersToSelectedUnit(getEstimatedVario());
        const char directionSymbol =
          verticalSpeed < 0 ? SYM_ARROW_SOUTH : SYM_ARROW_NORTH;
        tfp_sprintf(element->buff,
                    "%c%01d.%01d",
                    directionSymbol,
                    abs(verticalSpeed / 100),
                    abs((verticalSpeed % 100) / 10));
    } else {
        // We use this symbol when we don't have a valid measure
        element->buff[0] = SYM_COLON;
...
static void osdElementPidRateProfile(osdElementParms_t *element) {
    tfp_sprintf(element->buff,
                "%d-%d",
                getCurrentPidProfileIndex() + 1,
                getCurrentControlRateProfileIndex() + 1);
}
...
static void osdElementPower(osdElementParms_t *element) {
    tfp_sprintf(
      element->buff, "%4dW", getAmperage() * getBatteryVoltage() / 10000);
}
...
static void osdElementRemainingTimeEstimate(osdElementParms_t *element) {
    ...
    if (mAhDrawn <=
        0.1 *
          osdConfig()->cap_alarm) {  // also handles the mAhDrawn == 0 condition
        tfp_sprintf(element->buff, "--:--");
    } else if (mAhDrawn > osdConfig()->cap_alarm) {
        tfp_sprintf(element->buff, "00:00");
    } else {
        const int remaining_time = (int)((osdConfig()->cap_alarm - mAhDrawn) *
                                         ((float)osdFlyTime) / mAhDrawn);
...
static void osdElementStickOverlay(osdElementParms_t *element) {
    ...
    for (unsigned x = 0; x < OSD_STICK_OVERLAY_WIDTH; x++) {
        for (unsigned y = 0; y < OSD_STICK_OVERLAY_HEIGHT; y++) {
            ...
            if ((x == ((OSD_STICK_OVERLAY_WIDTH - 1) / 2)) &&
                (y == (OSD_STICK_OVERLAY_HEIGHT - 1) / 2)) {
                displayWriteChar(element->osdDisplayPort,
                                 xpos + x,
                                 ypos + y,
    ...
    if (element->item == OSD_STICK_OVERLAY_LEFT) {
        vertical_channel =
          radioModes[osdConfig()->overlay_radio_mode - 1].left_vertical;
        horizontal_channel =
          radioModes[osdConfig()->overlay_radio_mode - 1].left_horizontal;
    } else {
        vertical_channel =
          radioModes[osdConfig()->overlay_radio_mode - 1].right_vertical;
        horizontal_channel =
          radioModes[osdConfig()->overlay_radio_mode - 1].right_horizontal;
    }
    ...
    const uint8_t cursorY =
      OSD_STICK_OVERLAY_VERTICAL_POSITIONS - 1 -
      scaleRange(
    ...
    const char cursor = SYM_STICK_OVERLAY_SPRITE_HIGH +
                        (cursorY % OSD_STICK_OVERLAY_SPRITE_HEIGHT);

    displayWriteChar(element->osdDisplayPort,
                     xpos + cursorX,
                     ypos + cursorY / OSD_STICK_OVERLAY_SPRITE_HEIGHT,
                     cursor);
...
static void osdElementVtxChannel(osdElementParms_t *element) {
    ...
    const char vtxBandLetter =
      vtxCommonLookupBandLetter(vtxDevice, vtxSettingsConfig()->band);
    const char *vtxChannelName =
      vtxCommonLookupChannelName(vtxDevice, vtxSettingsConfig()->channel);
    ...
    tfp_sprintf(
      element->buff, "%c:%s:%1d", vtxBandLetter, vtxChannelName, vtxPower);
}
...
#define OSD_FORMAT_MESSAGE_BUFFER_SIZE (OSD_WARNINGS_MAX_SIZE + 1)
    ...
    STATIC_ASSERT(OSD_FORMAT_MESSAGE_BUFFER_SIZE <= OSD_ELEMENT_BUFFER_LENGTH,
    ...
    if (osdWarnGetState(OSD_WARNING_ARMING_DISABLE)) {
        if (IS_RC_MODE_ACTIVE(BOXARM) && isArmingDisabled()) {
            const armingDisableFlags_e armSwitchOnlyFlag =
              1 << (ARMING_DISABLE_FLAGS_COUNT - 1);
            ...
            // Rotate to the next arming disabled reason after a 0.5 second time
            // delay or if the current flag is no longer set
            if ((currentTimeUs - armingDisabledUpdateTimeUs > 5e5) ||
                !(flags & (1 << armingDisabledDisplayIndex))) {
            ...
            osdFormatMessage(
              ...
              armingDisableFlagNames[armingDisabledDisplayIndex]);
            return;
        } else {
    ...
    if (isTryingToArm() && !ARMING_FLAG(ARMED)) {
        int armingDelayTime = (getLastDshotBeaconCommandTimeUs() +
                               DSHOT_BEACON_GUARD_DELAY_US - currentTimeUs) /
                              1e5;
        ...
        if (armingDelayTime >= (DSHOT_BEACON_GUARD_DELAY_US / 1e5 - 5)) {
            ...
            tfp_sprintf(armingDelayMessage,
                        "ARM IN %d.%d",
                        armingDelayTime / 10,
                        armingDelayTime % 10);
            osdFormatMessage(element->buff,
                             OSD_FORMAT_MESSAGE_BUFFER_SIZE,
                             armingDelayMessage);
        }
        return;
    ...
    if (osdWarnGetState(OSD_WARNING_FAIL_SAFE) && failsafeIsActive()) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "FAIL SAFE");
        SET_BLINK(OSD_WARNINGS);
        return;
    }

    // Warn when in flip over after crash mode
    if (osdWarnGetState(OSD_WARNING_CRASH_FLIP) &&
        isFlipOverAfterCrashActive()) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "CRASH FLIP");
        return;
    }

#ifdef USE_LAUNCH_CONTROL
    // Warn when in launch control mode
    if (osdWarnGetState(OSD_WARNING_LAUNCH_CONTROL) &&
        isLaunchControlActive()) {
#ifdef USE_ACC
        if (sensors(SENSOR_ACC)) {
            char launchControlMsg[OSD_FORMAT_MESSAGE_BUFFER_SIZE];
            const int pitchAngle = constrain(
              (attitude.raw[FD_PITCH] -
               accelerometerConfig()->accelerometerTrims.raw[FD_PITCH]) /
                10,
              -90,
              90);
            tfp_sprintf(launchControlMsg, "LAUNCH %d", pitchAngle);
            osdFormatMessage(
              element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, launchControlMsg);
        } else
#endif  // USE_ACC
        {
            osdFormatMessage(
              element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "LAUNCH");
        }
        return;
    }
    ...
    // RSSI
    if (osdWarnGetState(OSD_WARNING_RSSI) &&
        (getRssiPercent() < osdConfig()->rssi_alarm)) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "RSSI LOW");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
...
#ifdef USE_RX_LINK_QUALITY_INFO
    // Link Quality
    if (osdWarnGetState(OSD_WARNING_LINK_QUALITY) &&
        (rxGetLinkQualityPercent() < osdConfig()->link_quality_alarm)) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "LINK QUALITY");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
#endif  // USE_RX_LINK_QUALITY_INFO

    if (osdWarnGetState(OSD_WARNING_BATTERY_CRITICAL) &&
        batteryState == BATTERY_CRITICAL) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, " LAND NOW");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_GPS_RESCUE_UNAVAILABLE) &&
        ARMING_FLAG(ARMED) && gpsRescueIsConfigured() &&
        !gpsRescueIsDisabled() && !gpsRescueIsAvailable()) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "RESCUE N/A");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_GPS_RESCUE_DISABLED) &&
        ...
        gpsRescueIsDisabled()) {
        statistic_t *stats = osdGetStats();
        if (cmpTimeUs(stats->armed_time,
                      OSD_GPS_RESCUE_DISABLED_WARNING_DURATION_US) < 0) {
            osdFormatMessage(
              element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "RESCUE OFF");
            SET_BLINK(OSD_WARNINGS);
            return;
        }
    }
    ...
    if (FLIGHT_MODE(HEADFREE_MODE)) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "HEADFREE");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_CORE_TEMPERATURE) &&
        coreTemperature >= osdConfig()->core_temp_alarm) {
        char coreTemperatureWarningMsg[OSD_FORMAT_MESSAGE_BUFFER_SIZE];
        tfp_sprintf(coreTemperatureWarningMsg,
                    "CORE %c: %3d%c",
                    SYM_TEMPERATURE,
...
#ifdef USE_ESC_SENSOR
    // Show warning if we lose motor output, the ESC is overheating or excessive
    // current draw
    if (featureIsEnabled(FEATURE_ESC_SENSOR) &&
        ...
        while (pos <
               (OSD_WARNINGS_MAX_SIZE - (strlen(title) + getMotorCount())) /
                 2) {
            escWarningMsg[pos++] = ' ';
        }
        ...
        while (i < getMotorCount() &&
               pos < OSD_FORMAT_MESSAGE_BUFFER_SIZE - 1) {
        ...
        if (escWarningCount > 0) {
            osdFormatMessage(
              element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, escWarningMsg);
            SET_BLINK(OSD_WARNINGS);
            return;
        }
    }
    ...
    if (osdWarnGetState(OSD_WARNING_BATTERY_WARNING) &&
        batteryState == BATTERY_WARNING) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "LOW BATTERY");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_RC_SMOOTHING) && ARMING_FLAG(ARMED) &&
        !rcSmoothingInitializationComplete()) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "RCSMOOTHING");
        SET_BLINK(OSD_WARNINGS);
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_BATTERY_NOT_FULL) &&
        !ARMING_FLAG(WAS_EVER_ARMED) && (getBatteryState() == BATTERY_OK) &&
        getBatteryAverageCellVoltage() < batteryConfig()->vbatfullcellvoltage) {
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "BATT < FULL");
        return;
    }
    ...
    if (osdWarnGetState(OSD_WARNING_VISUAL_BEEPER) &&
        ...
        osdFormatMessage(
          element->buff, OSD_FORMAT_MESSAGE_BUFFER_SIZE, "  * * * *");
        return;
    }
...
void osdUpdateAlarms(void) {
...
#ifdef USE_GPS_RESCUE
        || ((gpsSol.numSat < gpsRescueConfig()->minSats) &&
    ...
    for (int i = 0; i < OSD_TIMER_COUNT; i++) {
        ...
        const timeUs_t alarmTime =
          OSD_TIMER_ALARM(timer) * 60000000;  // convert from minutes to us
...
#ifdef USE_ESC_SENSOR
    if (featureIsEnabled(FEATURE_ESC_SENSOR)) {
        // This works because the combined ESC data contains the maximum
        // temperature seen amongst all ESCs
        if (osdConfig()->esc_temp_alarm != ESC_TEMP_ALARM_OFF &&
