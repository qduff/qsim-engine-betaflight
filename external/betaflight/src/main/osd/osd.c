...
const char *const osdTimerSourceNames[] = {"ON TIME  ",
                                           "TOTAL ARM",
                                           "LAST ARM ",
                                           "ON/ARM   "};
...
void pgResetFn_osdConfig(osdConfig_t *osdConfig) {
    ...
    osdConfig->core_temp_alarm =
      70;  // a temperature above 70C should produce a warning, lockups have
           // been reported above 80C
    ...
    for (int row = 0; row < 4; row++) {
        for (int column = 0; column < 24; column++) {
            if (fontOffset <= SYM_END_OF_FONT)
                displayWriteChar(
                  osdDisplayPort, x + column, y + row, fontOffset++);
        }
    }
}
    ...
    switch (blackboxConfig()->device) {
        ...
        case BLACKBOX_DEVICE_SDCARD:
            if (storageDeviceIsWorking) {
                ...
                storageUsed =
                  storageTotal - (afatfs_getContiguousFreeSpace() / 1024000);
            }
            break;
#endif
...
static void osdDisplayStatisticLabel(uint8_t y,
                                     const char *text,
                                     const char *value) {
...
// *** IMPORTANT ***
// The stats display order was previously required to match the enumeration
// definition so it matched the order shown in the configurator. However, to
// allow reordering this screen without breaking the compatibility, this
// requirement has been relaxed to a best effort approach. Reordering the
// elements on the stats screen will have to be more beneficial than the hassle
// of not matching exactly to the configurator list.
...
static uint8_t osdShowStats(uint16_t endBatteryVoltage, int statsRowCount) {
    ...
    // if statsRowCount is 0 then we're running an initial analysis of the
    // active stats items
    if (statsRowCount > 0) {
    ...
    if (osdStatGetState(OSD_STAT_TIMER_1)) {
        osdFormatTimer(
          buff,
          false,
          (OSD_TIMER_SRC(osdConfig()->timers[OSD_TIMER_1]) == OSD_TIMER_SRC_ON
             ? false
             : true),
          OSD_TIMER_1);
        osdDisplayStatisticLabel(
          top++,
          osdTimerSourceNames[OSD_TIMER_SRC(osdConfig()->timers[OSD_TIMER_1])],
          buff);
    }
    ...
    if (osdStatGetState(OSD_STAT_TIMER_2)) {
        osdFormatTimer(
          buff,
          false,
          (OSD_TIMER_SRC(osdConfig()->timers[OSD_TIMER_2]) == OSD_TIMER_SRC_ON
             ? false
             : true),
          OSD_TIMER_2);
        osdDisplayStatisticLabel(
          top++,
          osdTimerSourceNames[OSD_TIMER_SRC(osdConfig()->timers[OSD_TIMER_2])],
          buff);
    }
    ...
    if (featureIsEnabled(FEATURE_GPS)) {
        if (osdStatGetState(OSD_STAT_MAX_SPEED)) {
            tfp_sprintf(buff,
                        "%d%c",
                        osdGetSpeedToSelectedUnit(stats.max_speed),
        ...
        if (osdStatGetState(OSD_STAT_MAX_DISTANCE)) {
            tfp_sprintf(buff,
                        "%d%c",
                        osdGetMetersToSelectedUnit(stats.max_distance),
                        osdGetMetersToSelectedUnitSymbol());
        ...
        if (osdStatGetState(OSD_STAT_FLIGHT_DISTANCE)) {
            ...
            tfp_sprintf(buff,
                        "%d%c",
                        osdGetMetersToSelectedUnit(distanceFlown),
                        osdGetMetersToSelectedUnitSymbol());
...
#ifdef USE_BLACKBOX
    if (osdStatGetState(OSD_STAT_BLACKBOX) && blackboxConfig()->device &&
        blackboxConfig()->device != BLACKBOX_DEVICE_SERIAL) {
        osdGetBlackboxStatusString(buff);
        osdDisplayStatisticLabel(top++, "BLACKBOX", buff);
    }

    if (osdStatGetState(OSD_STAT_BLACKBOX_NUMBER) && blackboxConfig()->device &&
        blackboxConfig()->device != BLACKBOX_DEVICE_SERIAL) {
    ...
    if (osdStatGetState(OSD_STAT_MAX_ESC_TEMP)) {
        tfp_sprintf(buff,
                    "%d%c",
                    osdConvertTemperatureToSelectedUnit(stats.max_esc_temp),
...
#if defined(USE_GYRO_DATA_ANALYSE)
    if (osdStatGetState(OSD_STAT_MAX_FFT) &&
        featureIsEnabled(FEATURE_DYNAMIC_FILTER)) {
        int value = getMaxFFT();
        if (value > 0) {
...
#define METERS_PER_MILE 1609
        if (osdConfig()->units == OSD_UNIT_IMPERIAL) {
            tfp_sprintf(buff,
                        "%dMI",
                        statsConfig()->stats_total_dist_m / METERS_PER_MILE);
        } else {
            tfp_sprintf(
              buff,
              "%dKM",
              statsConfig()->stats_total_dist_m / METERS_PER_KILOMETER);
        }
...
static void osdRefreshStats(uint16_t endBatteryVoltage) {
    ...
    if (osdStatsRowCount == 0) {
        ...
        // Go through the logic one time to determine how many stats are
        // actually displayed.
        osdStatsRowCount = osdShowStats(endBatteryVoltage, 0);
        // Then clear the screen and commence with normal stats display which
        // will determine if the heading should be displayed and also center the
        // content vertically.
        displayClearScreen(osdDisplayPort);
    }
...
STATIC_UNIT_TESTED void osdRefresh(timeUs_t currentTimeUs) {
        ...
        if (ARMING_FLAG(ARMED)) {
            ...
            resumeRefreshAt = currentTimeUs + (REFRESH_1S / 2);
                   ...
                   (!(getArmingDisableFlags() &
                    ...
                    !VISIBLE(
                      osdConfig()->item_pos
                        [OSD_WARNINGS]))) {  // suppress stats if runaway
                                             // takeoff triggered disarm and
                                             // WARNINGS element is visible
            osdStatsEnabled = true;
            resumeRefreshAt = currentTimeUs + (60 * REFRESH_1S);
            endBatteryVoltage = getBatteryVoltage();
            osdStatsRowCount = 0;  // reset to 0 so it will be recalculated on
    ...
    } else if (osdStatsEnabled) {  // handle showing/hiding stats based on OSD
...
#if defined(USE_ACC)
    if (sensors(SENSOR_ACC) && (VISIBLE(osdConfig()->item_pos[OSD_G_FORCE]) ||
                                osdStatGetState(OSD_STAT_MAX_G_FORCE))) {
        // only calculate the G force if the element is visible or the stat is
        // enabled
...
#define STATS_FREQ_DENOM 50
    ...
    if (counter % DRAW_FREQ_DENOM == 0) {
        ...
        // rest of time redraw screen 10 chars per idle so it doesn't lock the
        // main idle
...
bool osdElementVisible(uint16_t value) {
    return (bool)((((value & OSD_PROFILE_MASK) >> OSD_PROFILE_BITS_POS) &
                   osdProfile) != 0);
}
#endif
