...
#if defined(STM32F7)
// #define HAL_CLEANINVALIDATECACHE(addr, size)
// (SCB_CleanInvalidateDCache_by_Addr((uint32_t*)((uint32_t)addr & ~0x1f),
// ((uint32_t)(addr + size + 0x1f) & ~0x1f) - ((uint32_t)addr & ~0x1f))) #define
// HAL_CLEANCACHE(addr, size)
// (SCB_CleanDCache_by_Addr((uint32_t*)((uint32_t)addr & ~0x1f),
// ((uint32_t)(addr + size + 0x1f) & ~0x1f) - ((uint32_t)addr & ~0x1f)))
...
#define DEFINE_DMA_CHANNEL(d, s, f)                               \
    {                                                             \
        .dma = d, .ref = d##_Stream##s, .stream = s,              \
        .irqHandlerCallback = NULL, .flagsShift = f,              \
        .irqN = d##_Stream##s##_IRQn, .userParam = 0, .owner = 0, \
        .resourceIndex = 0                                        \
    }
...
#define DMA_GET_FLAG_STATUS(d, flag)                                    \
    (d->flagsShift > 31 ? d->dma->HISR & (flag << (d->flagsShift - 32)) \
...
#define DEFINE_DMA_CHANNEL(d, c, f)                                     \
    {                                                                   \
        .dma = d, .ref = d##_Channel##c, .irqHandlerCallback = NULL,    \
        .flagsShift = f, .irqN = d##_Channel##c##_IRQn, .userParam = 0, \
        .owner = 0, .resourceIndex = 0                                  \
    }
...
void dmaInit(dmaIdentifier_e identifier,
             resourceOwner_e owner,
             uint8_t resourceIndex);
void dmaSetHandler(dmaIdentifier_e identifier,
