...
// FIXME remove this for targets that don't need a CLI.  Perhaps use a no-op
// macro when USE_CLI is not enabled signal that we're in cli mode
uint8_t cliMode = 0;
...
static const char *const mixerNames[] = {
  ...
  "BI",           "GIMBAL",      "Y6",
  "HEX6",         "FLYING_WING", "Y4",
  "HEX6X",        "OCTOX8",      "OCTOFLATP",
  "OCTOFLATX",    "AIRPLANE",    "HELI_120_CCPM",
  "HELI_90_DEG",  "VTAIL4",      "HEX6H",
  "PPM_TO_SERVO", "DUALCOPTER",  "SINGLECOPTER",
  "ATAIL4",       "CUSTOM",      "CUSTOMAIRPLANE",
  "CUSTOMTRI",    "QUADX1234",   NULL};
#endif

// sync this with features_e
static const char *const featureNames[] = {"RX_PPM",
                                           "",
                                           "INFLIGHT_ACC_CAL",
                                           "RX_SERIAL",
                                           "MOTOR_STOP",
...
static const rxFailsafeChannelMode_e
  rxFailsafeModesTable[RX_FAILSAFE_TYPE_COUNT][RX_FAILSAFE_MODE_COUNT] = {
...
#define SENSOR_NAMES_MASK \
    (SENSOR_GYRO | SENSOR_ACC | SENSOR_BARO | SENSOR_MAG | SENSOR_RANGEFINDER)
...
typedef bool printFn(dumpFlags_t dumpMask,
                     bool equalsDefault,
                     const char *format,
                     ...);
...
static bool cliDumpPrintLinef(dumpFlags_t dumpMask,
                              bool equalsDefault,
                              const char *format,
                              ...) {
...
static bool cliDefaultPrintLinef(dumpFlags_t dumpMask,
                                 bool equalsDefault,
                                 const char *format,
                                 ...) {
...
static void printValuePointer(const clivalue_t *var,
    ...
    if ((var->type & VALUE_MODE_MASK) == MODE_ARRAY) {
        ...
        switch (var->type & VALUE_MODE_MASK) {
            ...
            case MODE_LOOKUP:
                if (value <
                    lookupTables[var->config.lookup.tableIndex].valueCount) {
                    cliPrint(lookupTables[var->config.lookup.tableIndex]
                               .values[value]);
                } else {
                    valueIsCorrupted = true;
                }
...
static bool valuePtrEqualsDefault(const clivalue_t *var,
                                  const void *ptr,
    ...
    for (int i = 0; i < elementCount; i++) {
        switch (var->type & VALUE_TYPE_MASK) {
            case VAR_UINT8:
                result = result && (((uint8_t *)ptr)[i] & mask) ==
                                     (((uint8_t *)ptrDefault)[i] & mask);
                break;

            case VAR_INT8:
                result =
                  result && ((int8_t *)ptr)[i] == ((int8_t *)ptrDefault)[i];
                break;

            case VAR_UINT16:
                result = result && (((uint16_t *)ptr)[i] & mask) ==
                                     (((uint16_t *)ptrDefault)[i] & mask);
                break;
            case VAR_INT16:
                result =
                  result && ((int16_t *)ptr)[i] == ((int16_t *)ptrDefault)[i];
                break;
            case VAR_UINT32:
                result = result && (((uint32_t *)ptr)[i] & mask) ==
                                     (((uint32_t *)ptrDefault)[i] & mask);
                break;
        }
    }
...
static const char *cliPrintSectionHeading(dumpFlags_t dumpMask,
                                          bool outputFlag,
...
static uint8_t getPidProfileIndexToUse() {
    return pidProfileIndexToUse == CURRENT_PROFILE_INDEX
             ? getCurrentPidProfileIndex()
             : pidProfileIndexToUse;
}

static uint8_t getRateProfileIndexToUse() {
    return rateProfileIndexToUse == CURRENT_PROFILE_INDEX
...
static uint16_t getValueOffset(const clivalue_t *value) {
    switch (value->type & VALUE_SECTION_MASK) {
        ...
        case PROFILE_VALUE:
            return value->offset +
                   sizeof(pidProfile_t) * getPidProfileIndexToUse();
        case PROFILE_RATE_VALUE:
            return value->offset +
                   sizeof(controlRateConfig_t) * getRateProfileIndexToUse();
    }
    return 0;
}
...
static const char *dumpPgValue(const clivalue_t *value,
    ...
    const bool equalsDefault = valuePtrEqualsDefault(
      value, pg->copy + valueOffset, pg->address + valueOffset);
    ...
    if (((dumpMask & DO_DIFF) == 0) || !equalsDefault) {
        if (dumpMask & SHOW_DEFAULTS && !equalsDefault) {
            ...
            printValuePointer(
              value, (uint8_t *)pg->address + valueOffset, false);
            cliPrintLinefeed();
        }
...
static void dumpAllValues(uint16_t valueSection,
                          dumpFlags_t dumpMask,
                          const char *headingStr) {
    ...
    switch (var->type & VALUE_MODE_MASK) {
        case (MODE_DIRECT): {
            switch (var->type & VALUE_TYPE_MASK) {
                case VAR_UINT32:
                    cliPrintLinef(
                      "Allowed range: %d - %d", 0, var->config.u32Max);

                    break;
                case VAR_UINT8:
                case VAR_UINT16:
                    cliPrintLinef("Allowed range: %d - %d",
                                  var->config.minmaxUnsigned.min,
                                  var->config.minmaxUnsigned.max);

                    break;
                default:
                    cliPrintLinef("Allowed range: %d - %d",
                                  var->config.minmax.min,
                                  var->config.minmax.max);

                    break;
            }
        } break;
        case (MODE_LOOKUP): {
            const lookupTableEntry_t *tableEntry =
              &lookupTables[var->config.lookup.tableIndex];
...
static const char *processChannelRangeArgs(const char *ptr,
...
static void printRxFailsafe(
  dumpFlags_t dumpMask,
  const rxFailsafeChannelConfig_t *rxFailsafeChannelConfigs,
  const rxFailsafeChannelConfig_t *defaultRxFailsafeChannelConfigs,
  const char *headingStr) {
    // print out rxConfig failsafe settings
    headingStr = cliPrintSectionHeading(dumpMask, false, headingStr);
    for (uint32_t channel = 0; channel < MAX_SUPPORTED_RC_CHANNEL_COUNT;
         channel++) {
        const rxFailsafeChannelConfig_t *channelFailsafeConfig =
          &rxFailsafeChannelConfigs[channel];
        const rxFailsafeChannelConfig_t *defaultChannelFailsafeConfig =
          &defaultRxFailsafeChannelConfigs[channel];
        ...
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        if (requireValue) {
...
static void cliRxFailsafe(char *cmdline) {
    ...
    if (isEmpty(cmdline)) {
        ...
        if ((channel < MAX_SUPPORTED_RC_CHANNEL_COUNT)) {
            rxFailsafeChannelConfig_t *channelFailsafeConfig =
              rxFailsafeChannelConfigsMutable(channel);

            const rxFailsafeChannelType_e type =
              (channel < NON_AUX_CHANNEL_COUNT) ? RX_FAILSAFE_TYPE_FLIGHT
            ...
            cliShowArgumentRangeError(
              "CHANNEL", 0, MAX_SUPPORTED_RC_CHANNEL_COUNT - 1);
        }
    }
}

static void printAux(
  dumpFlags_t dumpMask,
  const modeActivationCondition_t *modeActivationConditions,
  const modeActivationCondition_t *defaultModeActivationConditions,
  const char *headingStr) {
    ...
    for (uint32_t i = 0; i < MAX_MODE_ACTIVATION_CONDITION_COUNT; i++) {
        ...
        if (defaultModeActivationConditions) {
            const modeActivationCondition_t *macDefault =
              &defaultModeActivationConditions[i];
            equalsDefault =
              !isModeActivationConditionConfigured(mac, macDefault);
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
...
static void cliAux(char *cmdline) {
    ...
    if (isEmpty(cmdline)) {
        ...
        if (i < MAX_MODE_ACTIVATION_CONDITION_COUNT) {
            ...
            ptr =
              processChannelRangeArgs(ptr, &mac->range, &validArgumentCount);
            ptr = nextArg(ptr);
            if (ptr) {
                val = atoi(ptr);
            ...
            cliShowArgumentRangeError(
              "INDEX", 0, MAX_MODE_ACTIVATION_CONDITION_COUNT - 1);
        }
    }
}

static void printSerial(dumpFlags_t dumpMask,
                        const serialConfig_t *serialConfig,
                        const serialConfig_t *serialConfigDefault,
                        const char *headingStr) {
    ...
    for (uint32_t i = 0; i < SERIAL_PORT_COUNT; i++) {
        ...
        if (serialConfigDefault) {
            equalsDefault = !memcmp(&serialConfig->portConfigs[i],
                                    &serialConfigDefault->portConfigs[i],
                                    sizeof(serialConfig->portConfigs[i]));
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
            cliDefaultPrintLinef(
              dumpMask,
              ...
              baudRates[serialConfigDefault->portConfigs[i]
    ...
    for (int i = 0; i < 4; i++) {
        ...
        switch (i) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (baudRateIndex < BAUD_19200 ||
                    baudRateIndex > BAUD_2470000) {
                    continue;
                }
...
static void cliSerialPassthrough(char *cmdline) {
    ...
    serialPortUsage_t *passThroughPortUsage =
    ...
    if (!passThroughPortUsage || passThroughPortUsage->serialPort == NULL) {
        ...
        passThroughPort = openSerialPort(
          id, FUNCTION_NONE, NULL, NULL, baud, mode, SERIAL_NOT_INVERTED);
        if (!passThroughPort) {
        ...
        if (baud) {
            ...
            cliPrintf("already open, baud = %d.\n\r",
                      passThroughPort->baudRate);
        }

        if (mode && passThroughPort->mode != mode) {
            cliPrintf(
              "Mode changed from %d to %d.\r\n", passThroughPort->mode, mode);
    ...
    if (enableBaudCb) {
        ...
        // Register the right side baud rate setting routine with the left side
        // which allows setting of the UART baud rate over USB without setting
        // it using the serialpassthrough command
    ...
    if (resetOnDtr
        ...
        serialSetCtrlLineStateCb(
          cliPort, cbCtrlLine, (void *)(intptr_t)(pinioDtr));
    }
...
static void printAdjustmentRange(
  dumpFlags_t dumpMask,
  const adjustmentRange_t *adjustmentRanges,
  const adjustmentRange_t *defaultAdjustmentRanges,
  const char *headingStr) {
...
static void cliAdjustmentRange(char *cmdline) {
    ...
    if (isEmpty(cmdline)) {
        ...
        if (i < MAX_ADJUSTMENT_RANGE_COUNT) {
            ...
            cliShowArgumentRangeError(
              "INDEX", 0, MAX_ADJUSTMENT_RANGE_COUNT - 1);
        }
    }
}

#ifndef USE_QUAD_MIXER_ONLY
static void printMotorMix(dumpFlags_t dumpMask,
                          const motorMixer_t *customMotorMixer,
                          const motorMixer_t *defaultCustomMotorMixer,
                          const char *headingStr) {
    ...
    for (uint32_t i = 0; i < MAX_SUPPORTED_MOTORS; i++) {
        if (customMotorMixer[i].throttle == 0.0f) break;
...
static void printRxRange(
  dumpFlags_t dumpMask,
  const rxChannelRangeConfig_t *channelRangeConfigs,
  const rxChannelRangeConfig_t *defaultChannelRangeConfigs,
  const char *headingStr) {
    ...
    for (uint32_t i = 0; i < NON_AUX_CHANNEL_COUNT; i++) {
        ...
        if (defaultChannelRangeConfigs) {
            equalsDefault = !memcmp(&channelRangeConfigs[i],
                                    &defaultChannelRangeConfigs[i],
                                    sizeof(channelRangeConfigs[i]));
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
            cliDefaultPrintLinef(dumpMask,
                                 equalsDefault,
    ...
    if (isEmpty(cmdline)) {
        ...
        if (i >= 0 && i < NON_AUX_CHANNEL_COUNT) {
            ...
            if (validArgumentCount != 2) {
                cliShowParseError();
            } else if (rangeMin < PWM_PULSE_MIN || rangeMin > PWM_PULSE_MAX ||
                       rangeMax < PWM_PULSE_MIN || rangeMax > PWM_PULSE_MAX) {
                cliShowParseError();
            } else {
                rxChannelRangeConfig_t *channelRangeConfig =
...
static void printLed(dumpFlags_t dumpMask,
                     const ledConfig_t *ledConfigs,
                     const ledConfig_t *defaultLedConfigs,
                     const char *headingStr) {
    ...
    for (uint32_t i = 0; i < LED_MAX_STRIP_LENGTH; i++) {
        ...
        if (defaultLedConfigs) {
            ...
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
            generateLedConfig(&ledConfigDefault,
                              ledConfigDefaultBuffer,
                              sizeof(ledConfigDefaultBuffer));
            cliDefaultPrintLinef(
              dumpMask, equalsDefault, format, i, ledConfigDefaultBuffer);
        }
...
static void cliLed(char *cmdline) {
    ...
    if (isEmpty(cmdline)) {
        printLed(
          DUMP_MASTER, ledStripStatusModeConfig()->ledConfigs, NULL, NULL);
    } else {
        ptr = cmdline;
        i = atoi(ptr);
        if (i < LED_MAX_STRIP_LENGTH) {
            ...
            if (parseLedStripConfig(i, ptr)) {
                generateLedConfig(
                  (ledConfig_t *)&ledStripStatusModeConfig()->ledConfigs[i],
                  ledConfigBuffer,
...
static void printColor(dumpFlags_t dumpMask,
                       const hsvColor_t *colors,
                       const hsvColor_t *defaultColors,
                       const char *headingStr) {
    ...
    for (uint32_t i = 0; i < LED_CONFIGURABLE_COLOR_COUNT; i++) {
        ...
        cliDumpPrintLinef(
          dumpMask, equalsDefault, format, i, color->h, color->s, color->v);
    }
}
        ...
        if (i < LED_CONFIGURABLE_COLOR_COUNT) {
            ...
            if (parseColor(i, ptr)) {
                const hsvColor_t *color =
                  &ledStripStatusModeConfig()->colors[i];
                cliDumpPrintLinef(
                  0, false, format, i, color->h, color->s, color->v);
            } else {
                cliShowParseError();
            }
        } else {
            cliShowArgumentRangeError(
              "INDEX", 0, LED_CONFIGURABLE_COLOR_COUNT - 1);
        }
    }
}

static void printModeColor(
  dumpFlags_t dumpMask,
  const ledStripStatusModeConfig_t *ledStripStatusModeConfig,
  const ledStripStatusModeConfig_t *defaultLedStripConfig,
  const char *headingStr) {
    ...
    for (uint32_t i = 0; i < LED_MODE_COUNT; i++) {
        for (uint32_t j = 0; j < LED_DIRECTION_COUNT; j++) {
            ...
            if (defaultLedStripConfig) {
                int colorIndexDefault =
                  defaultLedStripConfig->modeColors[i].color[j];
                equalsDefault = colorIndex == colorIndexDefault;
                headingStr =
                  cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
                cliDefaultPrintLinef(
                  dumpMask, equalsDefault, format, i, j, colorIndexDefault);
            }
            cliDumpPrintLinef(
              dumpMask, equalsDefault, format, i, j, colorIndex);
        }
    }
    ...
    for (uint32_t j = 0; j < LED_SPECIAL_COLOR_COUNT; j++) {
        ...
        bool equalsDefault = false;
        if (defaultLedStripConfig) {
            const int colorIndexDefault =
              defaultLedStripConfig->specialColors.color[j];
            equalsDefault = colorIndex == colorIndexDefault;
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
            cliDefaultPrintLinef(dumpMask,
        ...
        cliDumpPrintLinef(
          dumpMask, equalsDefault, format, LED_SPECIAL, j, colorIndex);
    }

    const int ledStripAuxChannel =
      ledStripStatusModeConfig->ledstrip_aux_channel;
    bool equalsDefault = false;
    if (defaultLedStripConfig) {
        const int ledStripAuxChannelDefault =
          defaultLedStripConfig->ledstrip_aux_channel;
    ...
    cliDumpPrintLinef(
      dumpMask, equalsDefault, format, LED_AUX_CHANNEL, 0, ledStripAuxChannel);
}
...
#ifdef USE_SERVOS
static void printServo(dumpFlags_t dumpMask,
                       const servoParam_t *servoParams,
                       const servoParam_t *defaultServoParams,
                       const char *headingStr) {
    ...
    for (uint32_t i = 0; i < MAX_SUPPORTED_SERVOS; i++) {
        ...
        if (defaultServoParams) {
            ...
            equalsDefault =
              !memcmp(servoConf, defaultServoConf, sizeof(*servoConf));
            headingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
            cliDefaultPrintLinef(dumpMask,
                                 equalsDefault,
    ...
    for (uint32_t i = 0; i < MAX_SUPPORTED_SERVOS; i++) {
        ...
        if (defaultServoParams) {
            bool equalsDefault =
              servoConf->reversedSources == servoConfDefault->reversedSources;
            for (uint32_t channel = 0; channel < INPUT_SOURCE_COUNT;
                 channel++) {
                equalsDefault = ~(servoConf->reversedSources ^
                                  servoConfDefault->reversedSources) &
                                (1 << channel);
                if (servoConfDefault->reversedSources & (1 << channel)) {
                    cliDefaultPrintLinef(
                      dumpMask, equalsDefault, format, i, channel);
                }
...
static void cliServo(char *cmdline) {
    ...
    if (isEmpty(cmdline)) {
        ...
        // Command line is integers (possibly negative) separated by spaces, no
        ...
        // Check we got the right number of args and the servo index is correct
        // (don't validate the other values)
        if (validArgumentCount != SERVO_ARGUMENT_COUNT || i < 0 ||
            i >= MAX_SUPPORTED_SERVOS) {
            cliShowParseError();
            return;
        }
        ...
        if (arguments[MIN] < PWM_PULSE_MIN || arguments[MIN] > PWM_PULSE_MAX ||
            arguments[MAX] < PWM_PULSE_MIN || arguments[MAX] > PWM_PULSE_MAX ||
            arguments[MIDDLE] < arguments[MIN] ||
            arguments[MIDDLE] > arguments[MAX] ||
            arguments[MIN] > arguments[MAX] ||
            arguments[MAX] < arguments[MIN] || arguments[RATE] < -100 ||
            arguments[RATE] > 100 ||
...
#ifdef USE_SERVOS
static void printServoMix(dumpFlags_t dumpMask,
                          const servoMixer_t *customServoMixers,
                          const servoMixer_t *defaultCustomServoMixers,
                          const char *headingStr) {
    ...
    for (uint32_t i = 0; i < MAX_SERVO_RULES; i++) {
        ...
        if (defaultCustomServoMixers) {
            ...
            equalsDefault = !memcmp(&customServoMixer,
...
static void cliServoMix(char *cmdline) {
    ...
    if (len == 0) {
        ...
        // erase custom mixer
        memset(
          customServoMixers_array(), 0, sizeof(*customServoMixers_array()));
        for (uint32_t i = 0; i < MAX_SUPPORTED_SERVOS; i++) {
        ...
        if (ptr == NULL) {
            cliPrintf("s");
            for (uint32_t inputSource = 0; inputSource < INPUT_SOURCE_COUNT;
                 inputSource++)
                cliPrintf("\ti%d", inputSource);
            cliPrintLinefeed();

            for (uint32_t servoIndex = 0; servoIndex < MAX_SUPPORTED_SERVOS;
                 servoIndex++) {
                cliPrintf("%d", servoIndex);
                for (uint32_t inputSource = 0; inputSource < INPUT_SOURCE_COUNT;
                     inputSource++) {
                    cliPrintf("\t%s  ",
                              (servoParams(servoIndex)->reversedSources &
                               (1 << inputSource))
                                ? "r"
                                : "n");
                }
        ...
        if (args[SERVO] >= 0 && args[SERVO] < MAX_SUPPORTED_SERVOS &&
            ...
            (*ptr == 'r' || *ptr == 'n')) {
            if (*ptr == 'r') {
                servoParamsMutable(args[SERVO])->reversedSources |=
                  1 << args[INPUT];
            } else {
                servoParamsMutable(args[SERVO])->reversedSources &=
                  ~(1 << args[INPUT]);
        ...
        int32_t i = args[RULE];
        if (i >= 0 && i < MAX_SERVO_RULES && args[TARGET] >= 0 &&
            args[TARGET] < MAX_SUPPORTED_SERVOS && args[INPUT] >= 0 &&
            args[INPUT] < INPUT_SOURCE_COUNT && args[RATE] >= -100 &&
            args[RATE] <= 100 && args[SPEED] >= 0 &&
            args[SPEED] <= MAX_SERVO_SPEED && args[MIN] >= 0 &&
            args[MIN] <= 100 && args[MAX] >= 0 && args[MAX] <= 100 &&
            args[MIN] < args[MAX] && args[BOX] >= 0 &&
            args[BOX] <= MAX_SERVO_BOXES) {
...
static void cliSdInfo(char *cmdline) {
    ...
    cliWriteBytes((uint8_t *)metadata->productName,
                  sizeof(metadata->productName));
...
static void cliFlashInfo(char *cmdline) {
    ...
    cliPrintLinef(
      "Flash sectors=%u, sectorSize=%u, pagesPerSector=%u, pageSize=%u, "
      "totalSize=%u, usedSize=%u",
      layout->sectors,
      layout->sectorSize,
      layout->pagesPerSector,
      layout->pageSize,
      layout->totalSize,
      flashfsGetOffset());
}
...
static void cliFlashRead(char *cmdline) {
    ...
    if (!nextArg) {
        ...
        while (length > 0) {
            ...
            if (bytesRead == 0) {
                // Assume we reached the end of the volume or something fatal
                // happened
                break;
            }
        }
...
static void printVtx(dumpFlags_t dumpMask,
                     const vtxConfig_t *vtxConfig,
                     const vtxConfig_t *vtxConfigDefault,
                     const char *headingStr) {
    ...
    for (uint32_t i = 0; i < MAX_CHANNEL_ACTIVATION_CONDITION_COUNT; i++) {
        const vtxChannelActivationCondition_t *cac =
          &vtxConfig->vtxChannelActivationConditions[i];
        if (vtxConfigDefault) {
            const vtxChannelActivationCondition_t *cacDefault =
              &vtxConfigDefault->vtxChannelActivationConditions[i];
    ...
    if (isEmpty(cmdline)) {
        ...
        if (i < MAX_CHANNEL_ACTIVATION_CONDITION_COUNT) {
            vtxChannelActivationCondition_t *cac =
              &vtxConfigMutable()->vtxChannelActivationConditions[i];
            uint8_t validArgumentCount = 0;
            ptr = nextArg(ptr);
            if (ptr) {
                val = atoi(ptr);
            ...
            ptr =
              processChannelRangeArgs(ptr, &cac->range, &validArgumentCount);
            ...
            cliShowArgumentRangeError(
              "INDEX", 0, MAX_CHANNEL_ACTIVATION_CONDITION_COUNT - 1);
        }
    }
}
...
static char *formatVtxTableBandFrequency(const uint16_t *frequency,
                                         int channels) {
...
static const char *printVtxTableBand(dumpFlags_t dumpMask,
                                     int band,
                                     const vtxTableConfig_t *currentConfig,
                                     const vtxTableConfig_t *defaultConfig,
                                     const char *headingStr) {
    ...
    if (defaultConfig) {
        equalsDefault = true;
        if (strcasecmp(currentConfig->bandNames[band],
                       defaultConfig->bandNames[band])) {
            equalsDefault = false;
        }
        if (currentConfig->bandLetters[band] !=
            defaultConfig->bandLetters[band]) {
            equalsDefault = false;
        }
        for (int channel = 0; channel < VTX_TABLE_MAX_CHANNELS; channel++) {
            if (currentConfig->frequency[band][channel] !=
                defaultConfig->frequency[band][channel]) {
                equalsDefault = false;
            }
        }
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        char *freqbuf = formatVtxTableBandFrequency(
          defaultConfig->frequency[band], defaultConfig->channels);
        cliDefaultPrintLinef(dumpMask,
                             equalsDefault,
                             fmt,
                             band + 1,
    ...
    char *freqbuf = formatVtxTableBandFrequency(currentConfig->frequency[band],
                                                currentConfig->channels);
    cliDumpPrintLinef(dumpMask,
                      equalsDefault,
                      fmt,
                      band + 1,
                      currentConfig->bandNames[band],
                      currentConfig->bandLetters[band],
                      freqbuf);
    return headingStr;
}
...
static const char *printVtxTablePowerValues(
  dumpFlags_t dumpMask,
  const vtxTableConfig_t *currentConfig,
  const vtxTableConfig_t *defaultConfig,
  const char *headingStr) {
    ...
    if (defaultConfig) {
        equalsDefault = true;
        for (int pwrindex = 0; pwrindex < VTX_TABLE_MAX_POWER_LEVELS;
             pwrindex++) {
            if (defaultConfig->powerValues[pwrindex] !=
                currentConfig->powerValues[pwrindex]) {
                equalsDefault = false;
            }
        }
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        char *pwrbuf = formatVtxTablePowerValues(defaultConfig->powerValues,
                                                 VTX_TABLE_MAX_POWER_LEVELS);
        cliDefaultPrintLinef(dumpMask, equalsDefault, fmt, pwrbuf);
    }

    char *pwrbuf = formatVtxTablePowerValues(currentConfig->powerValues,
                                             currentConfig->powerLevels);
    cliDumpPrintLinef(dumpMask, equalsDefault, fmt, pwrbuf);
    return headingStr;
}

static char *formatVtxTablePowerLabels(
  const char labels[VTX_TABLE_MAX_POWER_LEVELS]
                   [VTX_TABLE_POWER_LABEL_LENGTH + 1],
  int count) {
    static char
      pwrbuf[(VTX_TABLE_POWER_LABEL_LENGTH + 1) * VTX_TABLE_MAX_POWER_LEVELS +
             1];
...
static const char *printVtxTablePowerLabels(
  dumpFlags_t dumpMask,
  const vtxTableConfig_t *currentConfig,
  const vtxTableConfig_t *defaultConfig,
  const char *headingStr) {
    ...
    if (defaultConfig) {
        ...
        for (int pwrindex = 0; pwrindex < VTX_TABLE_MAX_POWER_LEVELS;
             pwrindex++) {
            if (strcasecmp(defaultConfig->powerLabels[pwrindex],
                           currentConfig->powerLabels[pwrindex])) {
                equalsDefault = false;
            }
        }
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        char *pwrbuf = formatVtxTablePowerLabels(defaultConfig->powerLabels,
                                                 VTX_TABLE_MAX_POWER_LEVELS);
        cliDefaultPrintLinef(dumpMask, equalsDefault, fmt, pwrbuf);
    }

    char *pwrbuf = formatVtxTablePowerLabels(currentConfig->powerLabels,
                                             currentConfig->powerLevels);
    cliDumpPrintLinef(dumpMask, equalsDefault, fmt, pwrbuf);
    return headingStr;
}

static void printVtxTable(dumpFlags_t dumpMask,
                          const vtxTableConfig_t *currentConfig,
                          const vtxTableConfig_t *defaultConfig,
                          const char *headingStr) {
    bool equalsDefault;
    ...
    if (defaultConfig) {
        ...
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        cliDefaultPrintLinef(
          dumpMask, equalsDefault, fmt, defaultConfig->bands);
    }
    ...
    if (defaultConfig) {
        ...
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        cliDefaultPrintLinef(
          dumpMask, equalsDefault, fmt, defaultConfig->channels);
    }
    ...
    for (int band = 0; band < currentConfig->bands; band++) {
        headingStr = printVtxTableBand(
          dumpMask, band, currentConfig, defaultConfig, headingStr);
    }
    ...
    if (defaultConfig) {
        equalsDefault =
          (defaultConfig->powerLevels == currentConfig->powerLevels);
        headingStr =
          cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
        cliDefaultPrintLinef(
          dumpMask, equalsDefault, fmt, defaultConfig->powerLevels);
    }
    ...
    // powerlabels
    headingStr = printVtxTablePowerValues(
      dumpMask, currentConfig, defaultConfig, headingStr);
    headingStr = printVtxTablePowerLabels(
      dumpMask, currentConfig, defaultConfig, headingStr);
}
...
static void cliVtxTable(char *cmdline) {
    ...
    if (!tok) {
        printVtxTable(
          DUMP_MASTER | HIDE_UNUSED, vtxTableConfigMutable(), NULL, NULL);
        return;
    }
    ...
    if (strcasecmp(tok, "bands") == 0) {
        ...
        if (bands < 0 || bands > VTX_TABLE_MAX_BANDS) {
            cliPrintErrorLinef(
              "INVALID BAND COUNT (SHOULD BE BETWEEN 0 AND %d)",
              VTX_TABLE_MAX_BANDS);
            return;
        }
        ...
        if (channels < 0 || channels > VTX_TABLE_MAX_BANDS) {
            cliPrintErrorLinef(
              "INVALID CHANNEL COUNT (SHOULD BE BETWEEN 0 AND %d)",
              VTX_TABLE_MAX_CHANNELS);
            return;
        }
        if (channels < vtxTableConfigMutable()->channels) {
            for (int i = 0; i < VTX_TABLE_MAX_BANDS; i++) {
                vtxTableConfigClearChannels(
                  vtxTableConfigMutable(), i, channels);
            }
        }
        ...
        if (tok) {
            ...
            if (levels < 0 || levels > VTX_TABLE_MAX_POWER_LEVELS) {
                cliPrintErrorLinef(
                  "INVALID POWER LEVEL COUNT (SHOULD BE BETWEEN 0 AND %d)",
                  VTX_TABLE_MAX_POWER_LEVELS);
            } else {
                if (levels < vtxTableConfigMutable()->powerLevels) {
                    vtxTableConfigClearPowerValues(vtxTableConfigMutable(),
                                                   levels);
                    vtxTableConfigClearPowerLabels(vtxTableConfigMutable(),
                                                   levels);
                }
        ...
        for (count = 0; count < levels && (tok = strtok_r(NULL, " ", &saveptr));
             count++) {
            int value = atoi(tok);
            power[count] = value;
        }
        ...
        // Power labels
        char label[VTX_TABLE_MAX_POWER_LEVELS]
                  [VTX_TABLE_POWER_LABEL_LENGTH + 1];
        int levels = vtxTableConfigMutable()->powerLevels;
        int count;
        for (count = 0; count < levels && (tok = strtok_r(NULL, " ", &saveptr));
        ...
        for (int i = 0; i < count; i++) {
            vtxTableStrncpyWithPad(vtxTableConfigMutable()->powerLabels[i],
                                   label[i],
        ...
        if (band < 0 || band >= bands) {
            cliPrintErrorLinef(
              "INVALID BAND NUMBER %s (EXPECTED 1-%d)", tok, bands);
            return;
        }
        ...
        for (channel = 0;
             channel < channels && (tok = strtok_r(NULL, " ", &saveptr));
             channel++) {
            int freq = atoi(tok);
            if (freq < 0) {
        ...
        if (channel < channels) {
            cliPrintErrorLinef("NOT ENOUGH FREQUENCIES (EXPECTED %d)",
                               channels);
            return;
        } else if ((tok = strtok_r(NULL, " ", &saveptr))) {
        ...
        vtxTableStrncpyWithPad(vtxTableConfigMutable()->bandNames[band],
                               bandname,
...
static void printName(dumpFlags_t dumpMask, const pilotConfig_t *pilotConfig) {
    ...
    cliDumpPrintLinef(dumpMask,
                      equalsDefault,
                      "\r\n# name: %s",
                      equalsDefault ? emptyName : pilotConfig->name);
}
...
static void cliName(char *cmdline) {
    ...
    if (len > 0) {
        ...
        if (strncmp(cmdline, emptyName, len)) {
            strncpy(
              pilotConfigMutable()->name, cmdline, MIN(len, MAX_NAME_LENGTH));
        }
        updated = true;
    }
    ...
    if (updated) {
        cliPrintLine(
          "###WARNING: This command will be removed. Use 'set name = ' "
          "instead.###");
    }
}
...
static void cliBoardName(char *cmdline) {
    const unsigned int len = strlen(cmdline);
    if (len > 0 && boardInformationIsSet() &&
        (len != strlen(getBoardName()) ||
         strncmp(getBoardName(), cmdline, len))) {
...
static void cliManufacturerId(char *cmdline) {
    const unsigned int len = strlen(cmdline);
    if (len > 0 && boardInformationIsSet() &&
        (len != strlen(getManufacturerId()) ||
         strncmp(getManufacturerId(), cmdline, len))) {
        cliPrintErrorLinef(
          ERROR_MESSAGE, "MANUFACTURER_ID", getManufacturerId());
    } else {
        if (len > 0) {
...
static void cliSignature(char *cmdline) {
    ...
    if (len > 0) {
        if (len != 2 * SIGNATURE_LENGTH) {
            cliPrintErrorLinef(
              "INVALID LENGTH: %d (EXPECTED: %d)", len, 2 * SIGNATURE_LENGTH);
...
#undef BLOCK_SIZE
    ...
    if (len > 0 && signatureIsSet() &&
        memcmp(signature, getSignature(), SIGNATURE_LENGTH)) {
...
static void printFeature(dumpFlags_t dumpMask,
                         const featureConfig_t *featureConfig,
                         const featureConfig_t *featureConfigDefault,
                         const char *headingStr) {
        ...
        if (strcmp(featureNames[i], emptyString) != 0) {  // Skip unused
            ...
            if (mask & (1 << i)) {
                ...
                headingStr =
                  cliPrintSectionHeading(dumpMask, !equalsDefault, headingStr);
                cliDumpPrintLinef(
                  dumpMask, equalsDefault, format, featureNames[i]);
            }
        }
    }
}
...
static void printBeeper(dumpFlags_t dumpMask,
    ...
    for (int32_t i = 0; i < beeperCount - 1; i++) {
        if (beeperModeMaskForTableIndex(i) & allowedFlags) {
            ...
            cliDefaultPrintLinef(
              ...
              offFlags & beeperModeMask ? formatOn : formatOff,
              name,
            ...
            cliDumpPrintLinef(dumpMask,
                              equalsDefault,
                              offFlags & beeperModeMask ? formatOff : formatOn,
                              name,
                              beeperNameForTableIndex(i));
        }
    }
...
static void processBeeperCommand(char *cmdline,
                                 uint32_t *offFlags,
                                 const uint32_t allowedFlags) {
    uint32_t len = strlen(cmdline);
        ...
        for (uint32_t i = 0;; i++) {
            ...
            if (strncasecmp(cmdline, beeperNameForTableIndex(i), len) == 0 &&
                beeperModeMaskForTableIndex(i) &
                  (allowedFlags | BEEPER_GET_FLAG(BEEPER_ALL))) {
...
void cliRxSpiBind(char *cmdline) {
  ...
  defined(USE_RX_FLYSKY) || defined(USE_RX_SPEKTRUM)
            rxSpiBind();
            cliPrint("Binding...");
            break;
#endif
...
static void printMap(dumpFlags_t dumpMask,
                     const rxConfig_t *rxConfig,
                     const rxConfig_t *defaultRxConfig,
                     const char *headingStr) {
    ...
    for (i = 0; i < RX_MAPPABLE_CHANNEL_COUNT; i++) {
        ...
        if (defaultRxConfig) {
            ...
            equalsDefault = equalsDefault &&
                            (rxConfig->rcmap[i] == defaultRxConfig->rcmap[i]);
        }
    }
    buf[i] = '\0';
...
static void cliMap(char *cmdline) {
    ...
    if (len == RX_MAPPABLE_CHANNEL_COUNT) {
        ...
        for (i = 0; i < RX_MAPPABLE_CHANNEL_COUNT; i++) {
            ...
            if (strchr(rcChannelLetters, buf[i]) &&
                !strchr(buf + i + 1, buf[i]))
                continue;
...
static char *checkCommand(char *cmdline, const char *command) {
    if (!strncasecmp(cmdline, command, strlen(command))  // command names match
        && (isspace((unsigned)cmdline[strlen(command)]) ||
            cmdline[strlen(command)] == 0)) {
...
static int parseOutputIndex(char *pch, bool allowAllEscs) {
    ...
    if ((outputIndex >= 0) && (outputIndex < getMotorCount())) {
        ...
        cliPrintErrorLinef("INVALID OUTPUT NUMBER. RANGE: 0 - %d.",
                           getMotorCount() - 1);
...
enum { ESC_INFO_KISS_V1, ESC_INFO_KISS_V2, ESC_INFO_BLHELI32 };
...
void printEscInfo(const uint8_t *escInfoBuffer, uint8_t bytesRead) {
    ...
    if (bytesRead > ESC_INFO_VERSION_POSITION) {
        ...
        if (bytesRead == frameLength) {
            ...
            if (calculateCrc8(escInfoBuffer, frameLength - 1) ==
                escInfoBuffer[frameLength - 1]) {
                ...
                switch (escInfoVersion) {
                    case ESC_INFO_KISS_V1:
                    case ESC_INFO_KISS_V2:
                        cliPrintLinef("Firmware Version: %d.%02d%c",
                                      firmwareVersion / 100,
                                      firmwareVersion % 100,
                        ...
                        break;
                    case ESC_INFO_BLHELI32:
                        cliPrintLinef("Firmware Version: %d.%02d%",
                                      firmwareVersion,
                                      firmwareSubVersion);

                        break;
                }
                if (escInfoVersion == ESC_INFO_KISS_V2 ||
                    escInfoVersion == ESC_INFO_BLHELI32) {
                    cliPrintLinef("Rotation Direction: %s",
                                  escInfoBuffer[16] ? "reversed" : "normal");
                    ...
                    if (escInfoVersion == ESC_INFO_BLHELI32) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            cliPrintLinef(
                              "LED %d: %s",
                              i,
                              setting ? (setting == 255) ? "unsupported" : "on"
                                      : "off");
                        }
                    }
                }
                ...
                if (command >= 0 && command < DSHOT_MIN_THROTTLE) {
                    if (firstCommand) {
                        ...
                        if (command == DSHOT_CMD_ESC_INFO) {
                            delay(5);  // Wait for potential ESC telemetry
                    ...
                    if (command != DSHOT_CMD_ESC_INFO) {
                        pwmWriteDshotCommand(
                          escIndex, getMotorCount(), command, true);
                    } else {
#if defined(USE_ESC_SENSOR) && defined(USE_ESC_SENSOR_INFO)
                    ...
                    cliPrintErrorLinef("INVALID COMMAND. RANGE: 1 - %d.",
...
static void cliProfile(char *cmdline) {
    if (isEmpty(cmdline)) {
        ...
        if (i >= 0 && i < PID_PROFILE_COUNT) {
            ...
            cliPrintErrorLinef("PROFILE OUTSIDE OF [0..%d]",
                               PID_PROFILE_COUNT - 1);
        }
    }
}
...
static void cliRateProfile(char *cmdline) {
    if (isEmpty(cmdline)) {
        ...
        if (i >= 0 && i < CONTROL_RATE_PROFILE_COUNT) {
            ...
            cliPrintErrorLinef("RATE PROFILE OUTSIDE OF [0..%d]",
...
uint16_t cliGetSettingIndex(char *name, uint8_t length) {
    for (uint32_t i = 0; i < valueTableEntryCount; i++) {
        ...
        // ensure exact match when setting to prevent setting variables with
        // shorter names
        if (strncasecmp(name, settingName, strlen(settingName)) == 0 &&
...
STATIC_UNIT_TESTED void cliSet(char *cmdline) {
    ...
    if (len == 0 || (len == 1 && cmdline[0] == '*')) {
        ...
        for (uint32_t i = 0; i < valueTableEntryCount; i++) {
            ...
            cliPrintf("%s = ", valueTable[i].name);
            cliPrintVar(val,
                        len);  // when len is 1 (when * is passed as argument),
                               // it will print min/max values as well, for gui
            cliPrintLinefeed();
        }
        ...
        switch (val->type & VALUE_MODE_MASK) {
            ...
            case MODE_BITSET: {
                ...
                const lookupTableEntry_t *tableEntry =
                  &lookupTables[tableIndex];
                bool matched = false;
                for (uint32_t tableValueIndex = 0;
                     tableValueIndex < tableEntry->valueCount && !matched;
                     tableValueIndex++) {
                    matched = tableEntry->values[tableValueIndex] &&
                              strcasecmp(tableEntry->values[tableValueIndex],
                                         eqptr) == 0;
            ...
            case MODE_ARRAY: {
                ...
                while (i < arrayLength && valPtr != NULL) {
                    ...
                    //       it stops at the first character that cannot be
                    //       converted...
                    switch (val->type & VALUE_TYPE_MASK) {
                        ...
                        case VAR_UINT8: {
                            // fetch data pointer
                            uint8_t *data =
                              (uint8_t *)cliGetValuePointer(val) + i;
                            // store value
                        ...
                        case VAR_INT8: {
                            // fetch data pointer
                            int8_t *data =
                              (int8_t *)cliGetValuePointer(val) + i;
                            // store value
                        ...
                        case VAR_UINT16: {
                            // fetch data pointer
                            uint16_t *data =
                              (uint16_t *)cliGetValuePointer(val) + i;
                            // store value
                        ...
                        case VAR_INT16: {
                            // fetch data pointer
                            int16_t *data =
                              (int16_t *)cliGetValuePointer(val) + i;
                            // store value
...
static void cliStatus(char *cmdline) {
    ...
    cliPrintf(" (%s%s)",
              ...
              (sysclkSource < 2) ? "" : PLLSource[pllSource]);
#endif

#ifdef USE_ADC_INTERNAL
    uint16_t vrefintMv = getVrefMv();
    int16_t coretemp = getCoreTemperatureCelsius();
    cliPrintLinef(", Vref=%d.%2dV, Core temp=%ddegC",
                  vrefintMv / 1000,
                  (vrefintMv % 1000) / 10,
                  coretemp);
#else
    ...
    cliPrintf(
      "Stack size: %d, Stack address: 0x%x", stackTotalSize(), stackHighMem());
#ifdef STACK_CHECK
...
#define CONFIG_SIZE (&__config_end - &__config_start)
#endif
    cliPrintLinef("Config size: %d, Max available config: %d",
                  getEEPROMConfigSize(),
                  CONFIG_SIZE);
    ...
    for (uint32_t i = 0;; i++) {
        ...
        if ((detectedSensorsMask & mask) && (mask & SENSOR_NAMES_MASK)) {
            ...
            const char *sensorHardware =
              sensorHardwareNames[i][sensorHardwareIndex];
            if (i) {
                cliPrint(", ");
            }
    ...
    const int gyroRate =
      getTaskDeltaTime(TASK_GYROPID) == 0
        ? 0
        : (int)(1000000.0f / ((float)getTaskDeltaTime(TASK_GYROPID)));
    const int rxRate = currentRxRefreshRate == 0
                         ? 0
                         : (int)(1000000.0f / ((float)currentRxRefreshRate));
    const int systemRate =
      getTaskDeltaTime(TASK_SYSTEM) == 0
        ? 0
        : (int)(1000000.0f / ((float)getTaskDeltaTime(TASK_SYSTEM)));
    cliPrintLinef(
      "CPU:%d%%, cycle time: %d, GYRO rate: %d, RX rate: %d, System rate: %d",
    ...
    cliPrintLinef("Voltage: %d * 0.01V (%dS battery - %s)",
                  getBatteryVoltage(),
                  getBatteryCellCount(),
...
static void cliTasks(char *cmdline) {
    ...
    if (systemConfig()->task_statistics) {
        cliPrintLine(
          "Task list             rate/hz  max/us  avg/us maxload avgload     "
          "total/ms");
    } else {
    ...
    for (cfTaskId_e taskId = 0; taskId < TASK_COUNT; taskId++) {
        ...
        if (taskInfo.isEnabled) {
            ...
            if (taskId == TASK_GYROPID) {
                subTaskFrequency =
                  taskInfo.movingAverageCycleTime == 0.0f
                    ? 0.0f
                    : (int)(1000000.0f / (taskInfo.movingAverageCycleTime));
                taskFrequency =
                  subTaskFrequency / pidConfig()->pid_process_denom;
                if (pidConfig()->pid_process_denom > 1) {
                    cliPrintf("%02d - (%15s) ", taskId, taskInfo.taskName);
                } else {
                    taskFrequency = subTaskFrequency;
                    cliPrintf("%02d - (%11s/%3s) ",
                              taskId,
                ...
                taskFrequency =
                  taskInfo.averageDeltaTime == 0
                    ? 0
                    : (int)(1000000.0f / ((float)taskInfo.averageDeltaTime));
                cliPrintf("%02d - (%15s) ", taskId, taskInfo.taskName);
            }
            const int maxLoad =
              taskInfo.maxExecutionTime == 0
                ? 0
                : (taskInfo.maxExecutionTime * taskFrequency + 5000) / 1000;
            const int averageLoad =
              taskInfo.averageExecutionTime == 0
                ? 0
                : (taskInfo.averageExecutionTime * taskFrequency + 5000) / 1000;
    ...
    if (systemConfig()->task_statistics) {
        ...
        cliPrintLinef("RX Check Function %19d %7d %25d",
                      checkFuncInfo.maxExecutionTime,
                      checkFuncInfo.averageExecutionTime,
                      checkFuncInfo.totalExecutionTime / 1000);
        cliPrintLinef("Total (excluding SERIAL) %25d.%1d%% %4d.%1d%%",
                      maxLoadSum / 10,
                      maxLoadSum % 10,
                      averageLoadSum / 10,
                      averageLoadSum % 10);
    ...
    if (rxConfig()->rc_smoothing_type == RC_SMOOTHING_TYPE_FILTER) {
        ...
        if (rcSmoothingAutoCalculate()) {
            ...
            if (avgRxFrameMs == 0) {
                ...
                cliPrintLinef(
                  "%d.%dms", avgRxFrameMs / 1000, avgRxFrameMs % 1000);
            }
        }
        cliPrint("# Input filter type: ");
        cliPrintLinef(lookupTables[TABLE_RC_SMOOTHING_INPUT_TYPE]
                        .values[rxConfig()->rc_smoothing_input_type]);
        cliPrintf("# Active input cutoff: %dhz ",
        ...
        cliPrintLinef(lookupTables[TABLE_RC_SMOOTHING_DERIVATIVE_TYPE]
                        .values[rxConfig()->rc_smoothing_derivative_type]);
        cliPrintf("# Active derivative cutoff: %dhz (",
...
const cliResourceValue_t resourceTable[] = {
  ...
  DEFS(OWNER_RX_SPI_CC2500_TX_EN,
       PG_RX_CC2500_SPI_CONFIG,
       rxCc2500SpiConfig_t,
       txEnIoTag),
  DEFS(OWNER_RX_SPI_CC2500_LNA_EN,
       PG_RX_CC2500_SPI_CONFIG,
       rxCc2500SpiConfig_t,
       lnaEnIoTag),
#if defined(USE_RX_CC2500_SPI_DIVERSITY)
  DEFS(OWNER_RX_SPI_CC2500_ANT_SEL,
       PG_RX_CC2500_SPI_CONFIG,
       rxCc2500SpiConfig_t,
       antSelIoTag),
#endif
...
static ioTag_t *getIoTag(const cliResourceValue_t value, uint8_t index) {
    const pgRegistry_t *rec = pgFind(value.pgn);
    return CONST_CAST(ioTag_t *,
                      rec->address + value.stride * index + value.offset);
}
...
static void printResource(dumpFlags_t dumpMask, const char *headingStr) {
    ...
    for (unsigned int i = 0; i < ARRAYLEN(resourceTable); i++) {
        ...
        for (int index = 0;
             index < MAX_RESOURCE_INDEX(resourceTable[i].maxIndex);
             index++) {
            const ioTag_t ioTag = *(ioTag_t *)((const uint8_t *)currentConfig +
                                               resourceTable[i].stride * index +
                                               resourceTable[i].offset);
            ioTag_t ioTagDefault = NULL;
            if (defaultConfig) {
                ioTagDefault = *(ioTag_t *)((const uint8_t *)defaultConfig +
                                            resourceTable[i].stride * index +
                                            resourceTable[i].offset);
            }
            ...
            if (ioTagDefault) {
                cliDefaultPrintLinef(dumpMask,
                                     ...
                                     IO_GPIOPortIdxByTag(ioTagDefault) + 'A',
                                     IO_GPIOPinIdxByTag(ioTagDefault));
...
static void resourceCheck(uint8_t resourceIndex,
                          uint8_t index,
                          ioTag_t newTag) {
    if (!newTag) {
        return;
    ...
    for (int r = 0; r < (int)ARRAYLEN(resourceTable); r++) {
        for (int i = 0; i < MAX_RESOURCE_INDEX(resourceTable[r].maxIndex);
             i++) {
            ...
            if (*tag == newTag) {
                ...
                cliPrintf(format,
                          DEFIO_TAG_GPIOID(newTag) + 'A',
                          DEFIO_TAG_PIN(newTag));
...
static void printPeripheralDmaoptDetails(dmaoptEntry_t *entry,
                                         int index,
    ...
    if (dmaopt != DMA_OPT_UNUSED) {
        ...
        const dmaChannelSpec_t *dmaChannelSpec =
          dmaGetChannelSpecByPeripheral(entry->peripheral, index, dmaopt);
        dmaCode_t dmaCode = 0;
        if (dmaChannelSpec) {
...
static const char *printPeripheralDmaopt(dmaoptEntry_t *entry,
                                         int index,
                                         dumpFlags_t dumpMask,
                                         const char *headingStr) {
    ...
    dmaoptValue_t currentOpt =
      *(dmaoptValue_t *)((uint8_t *)currentConfig + entry->stride * index +
                         entry->offset);
    dmaoptValue_t defaultOpt;

    if (defaultConfig) {
        defaultOpt = *(dmaoptValue_t *)((uint8_t *)defaultConfig +
                                        entry->stride * index + entry->offset);
    } else {
    ...
    if (defaultConfig) {
        printPeripheralDmaoptDetails(entry,
                                     index,
                                     defaultOpt,
                                     equalsDefault,
                                     dumpMask,
    ...
    printPeripheralDmaoptDetails(
      entry, index, currentOpt, equalsDefault, dumpMask, cliDumpPrintLinef);
    return headingStr;
}

#if defined(USE_TIMER_MGMT)
static void printTimerDmaoptDetails(const ioTag_t ioTag,
    ...
    if (dmaopt != DMA_OPT_UNUSED) {
        const bool printDetails = printValue(dumpMask,
                                             equalsDefault,
                                             format,
                                             IO_GPIOPortIdxByTag(ioTag) + 'A',
                                             IO_GPIOPinIdxByTag(ioTag),
        ...
        if (printDetails) {
            const dmaChannelSpec_t *dmaChannelSpec =
              dmaGetChannelSpecByTimerValue(timer->tim, timer->channel, dmaopt);
            dmaCode_t dmaCode = 0;
            if (dmaChannelSpec) {
                dmaCode = dmaChannelSpec->code;
                printValue(dumpMask,
...
static const char *printTimerDmaopt(const timerIOConfig_t *currentConfig,
    ...
    const timerHardware_t *timer =
      timerGetByTagAndIndex(ioTag, currentConfig[index].index);
    ...
    printTimerDmaoptDetails(
      ioTag, timer, dmaopt, equalsDefault, dumpMask, cliDumpPrintLinef);
    return headingStr;
}
#endif
...
static void printDmaopt(dumpFlags_t dumpMask, const char *headingStr) {
    ...
    for (size_t i = 0; i < ARRAYLEN(dmaoptEntryTable); i++) {
        ...
        for (int index = 0; index < entry->maxIndex; index++) {
            headingStr =
              printPeripheralDmaopt(entry, index, dumpMask, headingStr);
        }
    }
    ...
    for (unsigned i = 0; i < MAX_TIMER_PINMAP_COUNT; i++) {
        headingStr = printTimerDmaopt(
          currentConfig, defaultConfig, i, dumpMask, defaultIsUsed, headingStr);
    }

    if (defaultConfig) {
        for (unsigned i = 0; i < MAX_TIMER_PINMAP_COUNT; i++) {
            if (!defaultIsUsed[i] && defaultConfig[i].ioTag &&
                defaultConfig[i].dmaopt != DMA_OPT_UNUSED) {
                const timerHardware_t *timer = timerGetByTagAndIndex(
                  defaultConfig[i].ioTag, defaultConfig[i].index);
                headingStr = cliPrintSectionHeading(dumpMask, true, headingStr);
                printTimerDmaoptDetails(defaultConfig[i].ioTag,
                                        timer,
                                        defaultConfig[i].dmaopt,
                                        false,
                                        dumpMask,
                                        cliDefaultPrintLinef);
...
static void cliDmaopt(char *cmdline) {
    ...
    if (entry) {
        ...
        optaddr = (dmaoptValue_t *)((uint8_t *)currentConfig +
                                    entry->stride * index + entry->offset);
        orgval = *optaddr;
    } else {
        // It's a pin
    ...
    if (!pch) {
        if (entry) {
            printPeripheralDmaoptDetails(
              entry, index, *optaddr, true, DUMP_MASTER, cliDumpPrintLinef);
        } else {
            printTimerDmaoptDetails(
              ioTag, timer, orgval, true, DUMP_MASTER, cliDumpPrintLinef);
        }
        ...
        const dmaChannelSpec_t *dmaChannelSpec;
        if (entry) {
            for (int opt = 0; (dmaChannelSpec = dmaGetChannelSpecByPeripheral(
                                 entry->peripheral, index, opt));
                 opt++) {
                cliPrintLinef("# %d: DMA%d Stream %d channel %d",
                              opt,
                              DMA_CODE_CONTROLLER(dmaChannelSpec->code),
                              DMA_CODE_STREAM(dmaChannelSpec->code),
                              DMA_CODE_CHANNEL(dmaChannelSpec->code));
            }
        } else {
            for (int opt = 0; (dmaChannelSpec = dmaGetChannelSpecByTimerValue(
                                 timer->tim, timer->channel, opt));
                 opt++) {
                cliPrintLinef("# %d: DMA%d Stream %d channel %d",
                              opt,
                              DMA_CODE_CONTROLLER(dmaChannelSpec->code),
        ...
        if (strcasecmp(pch, "none") == 0) {
            ...
            if (entry) {
                if (!dmaGetChannelSpecByPeripheral(
                      entry->peripheral, index, optval)) {
                    cliPrintErrorLinef("INVALID DMA OPTION FOR %s %d: '%s'",
                                       entry->device,
                                       DMA_OPT_UI_INDEX(index),
                                       pch);
                ...
                if (!dmaGetChannelSpecByTimerValue(
                      timer->tim, timer->channel, optval)) {
                    cliPrintErrorLinef(
                      "INVALID DMA OPTION FOR PIN %c%02d: '%s'",
                      IO_GPIOPortIdxByTag(ioTag) + 'A',
                      IO_GPIOPinIdxByTag(ioTag),
                      pch);
        ...
        if (optval != orgval) {
            if (entry) {
                *optaddr = optval;
                ...
                cliPrintLinef("dma %s %d: changed from %s to %s",
                              entry->device,
                              DMA_OPT_UI_INDEX(index),
                              orgvalString,
                              optvalString);
                ...
                cliPrintLinef("dma pin %c%02d: changed from %s to %s",
                              IO_GPIOPortIdxByTag(ioTag) + 'A',
                              IO_GPIOPinIdxByTag(ioTag),
                              orgvalString,
                              optvalString);
            ...
            if (entry) {
                cliPrintLinef("dma %s %d: no change: %s",
                              entry->device,
                              DMA_OPT_UI_INDEX(index),
                              orgvalString);
            } else {
                cliPrintLinef("dma %c%02d: no change: %s",
                              IO_GPIOPortIdxByTag(ioTag) + 'A',
                              IO_GPIOPinIdxByTag(ioTag),
                              orgvalString);
            }
...
static void cliResource(char *cmdline) {
    ...
    if (!pch) {
        ...
        for (int i = 0; i < DEFIO_IO_USED_COUNT; i++) {
            ...
            cliPrintf("%c%02d: %s",
                      IO_GPIOPortIdx(ioRecs + i) + 'A',
                      IO_GPIOPinIdx(ioRecs + i),
                      owner);
            if (ioRecs[i].index > 0) {
    ...
    for (resourceIndex = 0;; resourceIndex++) {
        ...
        const char *resourceName =
          ownerNames[resourceTable[resourceIndex].owner];
    ...
    if (resourceTable[resourceIndex].maxIndex > 0 || index > 0) {
        if (index <= 0 ||
            index > MAX_RESOURCE_INDEX(resourceTable[resourceIndex].maxIndex)) {
            cliShowArgumentRangeError(
              "INDEX",
              1,
              MAX_RESOURCE_INDEX(resourceTable[resourceIndex].maxIndex));
            return;
        }
        index -= 1;
...
#ifdef MINIMAL_CLI
                    cliPrintLinef(" %c%02d set",
                                  IO_GPIOPortIdx(rec) + 'A',
                                  IO_GPIOPinIdx(rec));
#else
...
static void printTimerDetails(const ioTag_t ioTag,
...
static void printTimer(dumpFlags_t dumpMask, const char *headingStr) {
    ...
    for (unsigned int i = 0; i < MAX_TIMER_PINMAP_COUNT; i++) {
        ...
        printTimerDetails(
          ioTag, timerIndex, equalsDefault, dumpMask, cliDumpPrintLinef);
    }
    ...
    if (defaultConfig) {
        for (unsigned i = 0; i < MAX_TIMER_PINMAP_COUNT; i++) {
            if (!defaultIsUsed[i] && defaultConfig[i].ioTag) {
                ...
                printTimerDetails(defaultConfig[i].ioTag,
                                  defaultConfig[i].index,
                                  false,
                                  dumpMask,
                                  cliDefaultPrintLinef);
    ...
    if (pch) {
        ...
        if (strcasecmp(pch, "list") == 0) {
            ...
            for (unsigned index = 0;
                 (timer = timerGetByTagAndIndex(ioTag, index + 1));
                 index++) {
            ...
            const timerHardware_t *timer =
              timerGetByTagAndIndex(ioTag, timerIndex + 1);

            if (!timer) {
                cliPrintErrorLinef("INVALID TIMER OPTION FOR %c%02d: '%s'",
                                   IO_GPIOPortIdxByTag(ioTag) + 'A',
                                   IO_GPIOPinIdxByTag(ioTag),
                                   pch);
        ...
        int oldTimerIndex =
          isExistingTimerOpt ? timerIOConfig(timerIOIndex)->index - 1 : -1;
        timerIOConfigMutable(timerIOIndex)->ioTag =
          timerIndex == TIMER_INDEX_UNDEFINED ? IO_TAG_NONE : ioTag;
        ...
        if (timerIndex == oldTimerIndex) {
            cliPrintLinef("timer %c%02d: no change: %s",
                          IO_GPIOPortIdxByTag(ioTag) + 'A',
                          IO_GPIOPinIdxByTag(ioTag),
                          orgvalString);
        } else {
            cliPrintLinef("timer %c%02d: changed from %s to %s",
                          IO_GPIOPortIdxByTag(ioTag) + 'A',
                          IO_GPIOPinIdxByTag(ioTag),
                          orgvalString,
                          optvalString);
        ...
        printTimerDetails(ioTag,
                          timerIOConfig(timerIOIndex)->index,
                          false,
                          DUMP_MASTER,
                          cliDumpPrintLinef);
    ...
    if (useDshotTelemetry) {
        ...
        for (uint8_t i = 0; i < getMotorCount(); i++) {
            ...
            if (isDshotMotorTelemetryActive(i)) {
                ...
                cliPrintLinef(
                  "%3d.%02d%%", calcPercent / 100, calcPercent % 100);
            } else {
        ...
        const int modulo =
          proshot ? MOTOR_NIBBLE_LENGTH_PROSHOT : MOTOR_BITLENGTH;
        ...
        for (int i = 1; i < len; i += 2) {
            cliPrintf(
              "%u ",
              (int)(inputBuffer[i] + modulo - inputBuffer[i - 1]) % modulo);
        }
        cliPrintLinefeed();
    } else {
...
static void printConfig(char *cmdline, bool doDiff) {
    ...
    if ((options = checkCommand(cmdline, "master"))) {
        ...
        dumpMask =
          DUMP_MASTER |
          HARDWARE_ONLY;  // Show only hardware related settings (useful to
                          // generate unified target configs).
    ...
    if (checkCommand(options, "defaults")) {
        dumpMask =
          dumpMask |
          SHOW_DEFAULTS;  // add default values as comments for changed values
    } else if (checkCommand(options, "bare")) {
        dumpMask = dumpMask | BARE;  // show the diff / dump without extra
                                     // commands and board specific data
    }
    ...
    if ((dumpMask & DUMP_MASTER) || (dumpMask & DUMP_ALL)) {
        ...
        if (!(dumpMask & HARDWARE_ONLY)) {
            ...
            const bool equalsDefault =
              mixerConfig_Copy.mixerMode == mixerConfig()->mixerMode;
            mixerHeadingStr =
              cliPrintSectionHeading(dumpMask, !equalsDefault, mixerHeadingStr);
            ...
            cliDumpPrintLinef(dumpMask,
                              equalsDefault,
                              formatMixer,
                              mixerNames[mixerConfig_Copy.mixerMode - 1]);

            cliDumpPrintLinef(dumpMask,
                              customMotorMixer(0)->throttle == 0.0f,
                              "\r\nmmix reset\r\n");
...
#ifdef USE_SERVOS
            printServo(
              dumpMask, servoParams_CopyArray, servoParams(0), "servo");
            ...
            printFeature(
              dumpMask, &featureConfig_Copy, featureConfig(), "feature");

#if defined(USE_BEEPER)
            printBeeper(dumpMask,
                        beeperConfig_Copy.beeper_off_flags,
...
#if defined(USE_DSHOT)
            printBeeper(dumpMask,
                        beeperConfig_Copy.dshotBeaconOffFlags,
...
#ifdef USE_LED_STRIP_STATUS_MODE
            printLed(dumpMask,
                     ledStripStatusModeConfig_Copy.ledConfigs,
                     ledStripStatusModeConfig()->ledConfigs,
                     "led");

            printColor(dumpMask,
                       ledStripStatusModeConfig_Copy.colors,
                       ledStripStatusModeConfig()->colors,
                       "color");

            printModeColor(dumpMask,
...
#ifdef USE_VTX_TABLE
            printVtxTable(
              dumpMask, &vtxTableConfig_Copy, vtxTableConfig(), "vtxtable");
#endif
        ...
        if (dumpMask & HARDWARE_ONLY) {
            ...
            if (dumpMask & DUMP_ALL) {
                for (uint32_t pidProfileIndex = 0;
                     pidProfileIndex < PID_PROFILE_COUNT;
                     pidProfileIndex++) {
                ...
                for (uint32_t rateIndex = 0;
                     rateIndex < CONTROL_RATE_PROFILE_COUNT;
                     rateIndex++) {
                ...
                cliDumpRateProfile(systemConfig_Copy.activeRateProfile,
                                   dumpMask);
            }
        }
...
static void cliMsc(char *cmdline) {
    if (mscCheckFilesystemReady()) {
        ...
        if (!isEmpty(cmdline)) {
            ...
            if ((timezoneOffsetMinutes < TIMEZONE_OFFSET_MINUTES_MIN) ||
                (timezoneOffsetMinutes > TIMEZONE_OFFSET_MINUTES_MAX)) {
...
const clicmd_t cmdTable[] = {
...
#if defined(USE_BEEPER)
#if defined(USE_DSHOT)
  CLI_COMMAND_DEF("beacon",
                  "enable/disable Dshot beacon for a condition",
                  "list\r\n"
                  "\t<->[name]",
                  cliBeacon),
#endif
  CLI_COMMAND_DEF("beeper",
                  "enable/disable beeper for a condition",
                  "list\r\n"
                  "\t<->[name]",
                  cliBeeper),
#endif  // USE_BEEPER
#ifdef USE_RX_SPI
  CLI_COMMAND_DEF("bind_rx_spi",
                  "initiate binding for RX SPI",
                  NULL,
                  cliRxSpiBind),
#endif
  CLI_COMMAND_DEF("bl", "reboot into bootloader", NULL, cliBootloader),
#if defined(USE_BOARD_INFO)
  CLI_COMMAND_DEF("board_name",
                  "get / set the name of the board model",
                  "[board name]",
                  cliBoardName),
#endif
#ifdef USE_LED_STRIP_STATUS_MODE
...
#endif
  CLI_COMMAND_DEF("defaults",
                  "reset to defaults and reboot",
                  "[nosave]",
                  cliDefaults),
  CLI_COMMAND_DEF("diff",
...
#ifdef USE_DMA_SPEC
  CLI_COMMAND_DEF("dma",
                  "show/set DMA assignments",
                  "<> | <device> <index> list | <device> <index> "
                  "[<option>|none] | list | show",
                  cliDma),
#else
...
#endif
#ifdef USE_DSHOT_TELEMETRY
  CLI_COMMAND_DEF("dshot_telemetry_info",
                  "disply dshot telemetry info and stats",
                  NULL,
                  cliDshotTelemetryInfo),
#endif
#ifdef USE_DSHOT
  CLI_COMMAND_DEF("dshotprog",
                  "program DShot ESC(s)",
                  "<index> <command>+",
                  cliDshotProg),
#endif
  CLI_COMMAND_DEF("dump",
                  "dump configuration",
                  "[master|profile|rates|hardware|all] {defaults|bare}",
                  cliDump),
#ifdef USE_ESCSERIAL
  CLI_COMMAND_DEF("escprog",
                  "passthrough esc to serial",
                  "<mode [sk/bl/ki/cc]> <index>",
                  cliEscPassthrough),
#endif
  CLI_COMMAND_DEF("exit", NULL, NULL, cliExit),
  CLI_COMMAND_DEF("feature",
                  "configure features",
                  "list\r\n"
                  "\t<->[name]",
                  cliFeature),
...
#ifdef USE_GPS
  CLI_COMMAND_DEF("gpspassthrough",
                  "passthrough gps to serial",
                  NULL,
                  cliGpsPassthrough),
#endif
#if defined(USE_GYRO_REGISTER_DUMP) && !defined(SIMULATOR_BUILD)
  CLI_COMMAND_DEF("gyroregisters",
                  "dump gyro config registers contents",
                  NULL,
...
#endif
#if defined(USE_BOARD_INFO)
  CLI_COMMAND_DEF("manufacturer_id",
                  "get / set the id of the board manufacturer",
                  "[manufacturer id]",
                  cliManufacturerId),
#endif
  ...
  CLI_COMMAND_DEF("rateprofile",
                  "change rate profile",
                  "[<index>]",
                  cliRateProfile),
#ifdef USE_RC_SMOOTHING_FILTER
  CLI_COMMAND_DEF("rc_smoothing_info",
                  "show rc_smoothing operational settings",
                  NULL,
                  cliRcSmoothing),
#endif  // USE_RC_SMOOTHING_FILTER
#ifdef USE_RESOURCE_MGMT
  CLI_COMMAND_DEF("resource",
                  "show/set resources",
                  "<> | <resource name> <index> [<pin>|none] | show [all]",
                  cliResource),
#endif
  CLI_COMMAND_DEF("rxfail",
  ...
  CLI_COMMAND_DEF("serialpassthrough",
                  "passthrough serial data to port",
                  "<id> [baud] [mode] [dtr pinio|'reset']",
  ...
  CLI_COMMAND_DEF("signature",
                  "get / set the board type signature",
                  "[signature]",
                  cliSignature),
#endif
#ifdef USE_SERVOS
  CLI_COMMAND_DEF("smix",
                  "servo mixer",
...
#endif
#ifdef USE_TIMER_MGMT
  CLI_COMMAND_DEF("timer",
                  "show/set timers",
                  "<> | <pin> list | <pin> [<option>|none] | list | show",
                  cliTimer),
#endif
...
#else
  CLI_COMMAND_DEF("vtx",
                  "vtx channels on switch",
                  "<index> <aux_channel> <vtx_band> <vtx_channel> <vtx_power> "
                  "<start_range> <end_range>",
                  cliVtx),
#endif
#endif
#ifdef USE_VTX_TABLE
  CLI_COMMAND_DEF("vtxtable",
                  "vtx frequency able",
                  "<band> <bandname> <bandletter> <freq> ... <freq>\r\n",
                  cliVtxTable),
#endif
};
    ...
    while (serialRxBytesWaiting(cliPort)) {
        ...
        if (c == '\t' || c == '?') {
            ...
            for (cmd = cmdTable; cmd < cmdTable + ARRAYLEN(cmdTable); cmd++) {
                if (bufferIndex &&
                    (strncasecmp(cliBuffer, cmd->name, bufferIndex) != 0))
                    continue;
            ...
            if (pstart) { /* Buffer matches one or more commands */
                for (;; bufferIndex++) {
                    ...
                    if (!pstart->name[bufferIndex] &&
                        bufferIndex < sizeof(cliBuffer) - 2) {
            ...
            if (bufferIndex > 0) {
                ...
                const clicmd_t *cmd;
                char *options;
                for (cmd = cmdTable; cmd < cmdTable + ARRAYLEN(cmdTable);
                     cmd++) {
            ...
            // 'exit' will reset this flag, so we don't need to print prompt
            // again
            if (!cliMode) return;
            ...
            if (!bufferIndex && c == ' ') continue;  // Ignore leading spaces
...
void cliEnter(serialPort_t *serialPort) {
    ...
    cliWriter = bufWriterInit(cliWriteBuffer,
