...
#define CRC_CHECK_VALUE \
    0x1D0F  // pre-calculated value of CRC that includes the CRC itself
...
typedef struct {
    ...
    // lower 2 bits used to indicate system or profile number, see
...
// checksum is appended just after footer. It is not included in footer to make
// checksum calculation consistent
...
bool isEEPROMStructureValid(void) {
    ...
    // CRC has the property that if the CRC itself is included in the
    // calculation the resulting CRC will have constant value
...
static const configRecord_t *findEEPROM(const pgRegistry_t *reg,
    ...
    while (true) {
        const configRecord_t *record = (const configRecord_t *)p;
        if (record->size == 0 || p + record->size >= &__config_end ||
            record->size < sizeof(*record))
            break;
...
// This functions processes all PGs sequentially, scanning EEPROM for each one.
// This is suboptimal,
...
bool loadEEPROM(void) {
    ...
    PG_FOREACH(reg) {
        ...
        if (rec) {
            // config from EEPROM is available, use it to initialize PG. pgLoad
            // will handle version mismatch
            if (!pgLoad(reg,
                        rec->pg,
                        rec->size - offsetof(configRecord_t, pg),
                        rec->version)) {
                success = false;
            }
        } else {
...
static bool writeSettingsToEEPROM(void) {
    ...
    config_streamer_start(
      &streamer, (uintptr_t)&__config_start, &__config_end - &__config_start);
    ...
    config_streamer_write(
      &streamer, (uint8_t *)&invertedBigEndianCrc, sizeof(crc));
