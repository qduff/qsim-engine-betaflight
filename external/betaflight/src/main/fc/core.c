...
#define RUNAWAY_TAKEOFF_PIDSUM_THRESHOLD \
    600  // The pidSum threshold required to trigger - corresponds to a pidSum
         // value of 60% (raw 600) in the blackbox viewer
#define RUNAWAY_TAKEOFF_ACTIVATE_DELAY \
    75000  // (75ms) Time in microseconds where pidSum is above threshold to
           // trigger
#define RUNAWAY_TAKEOFF_DEACTIVATE_STICK_PERCENT \
    15  // 15% - minimum stick deflection during deactivation phase
#define RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT \
    100  // 10.0% - pidSum limit during deactivation phase
#define RUNAWAY_TAKEOFF_GYRO_LIMIT_RP \
    15  // Roll/pitch 15 deg/sec threshold to prevent triggering during bench
        // testing without props
#define RUNAWAY_TAKEOFF_GYRO_LIMIT_YAW \
    50  // Yaw 50 deg/sec threshold to prevent triggering during bench testing
        // without props
#define RUNAWAY_TAKEOFF_HIGH_THROTTLE_PERCENT \
    75  // High throttle limit to accelerate deactivation (halves the
        // deactivation delay)
...
static timeUs_t
  disarmAt;  // Time of automatic disarm when "Don't spin the motors when armed"
             // is enabled and auto_disarm_delay is nonzero
...
const char* const osdLaunchControlModeNames[] = {"NORMAL", "PITCHONLY", "FULL"};
#endif
...
static bool isCalibrating(void) {
    ...
    // Note: compass calibration is handled completely differently, outside of
    // the main loop, see f.CALIBRATE_MAG
...
void updateArmingStatus(void) {
    if (ARMING_FLAG(ARMED)) {
        ...
        // If switch is used for arming then check it is not defaulting to on
        // when the RX link recovers from a fault
        if (!isUsingSticksForArming()) {
            ...
            if (justGotRxBack && IS_RC_MODE_ACTIVE(BOXARM)) {
                // If the RX has just started to receive a signal again and the
                // arm switch is on, apply arming restriction
                setArmingDisabled(ARMING_DISABLED_BAD_RX_RECOVERY);
            } else if (haveRx && !IS_RC_MODE_ACTIVE(BOXARM)) {
                // If RX signal is OK and the arm switch is off, remove arming
                // restriction
        ...
        // USE_RPM_FILTER will only be defined if USE_DSHOT and
        // USE_DSHOT_TELEMETRY are defined If the RPM filter is anabled and any
        // motor isn't providing telemetry, then disable arming
        ...
        if (!isUsingSticksForArming()) {
            /* Ignore ARMING_DISABLED_CALIBRATING if we are going to calibrate
             * gyro on first arm */
            bool ignoreGyro =
              armingConfig()->gyro_cal_on_first_arm &&
              !(getArmingDisableFlags() &
                ~(ARMING_DISABLED_ARM_SWITCH | ARMING_DISABLED_CALIBRATING));

            /* Ignore ARMING_DISABLED_THROTTLE (once arm switch is on) if we are
             * in 3D mode */
            bool ignoreThrottle =
              featureIsEnabled(FEATURE_3D) && !IS_RC_MODE_ACTIVE(BOX3D) &&
              !flight3DConfig()->switched_mode3d &&
              !(getArmingDisableFlags() &
                ~(ARMING_DISABLED_ARM_SWITCH | ARMING_DISABLED_THROTTLE));
            ...
            if (isArmingDisabled() && !ignoreGyro && !ignoreThrottle &&
...
void disarm(void) {
    if (ARMING_FLAG(ARMED)) {
        ...
        if (blackboxConfig()->device &&
            blackboxConfig()->mode !=
              BLACKBOX_MODE_ALWAYS_ON) {  // Close the log upon disarm except
                                          // when logging mode is ALWAYS ON
            blackboxFinish();
        }
#endif
        BEEP_OFF;
        ...
        if (isMotorProtocolDshot() && flipOverAfterCrashActive &&
            ...
            pwmWriteDshotCommand(ALL_MOTORS,
                                 getMotorCount(),
        ...
        // if ARMING_DISABLED_RUNAWAY_TAKEOFF is set then we want to play it's
        // beep pattern instead
...
void tryArm(void) {
    ...
    if (!isArmingDisabled()) {
...
#ifdef USE_DSHOT
        if (currentTimeUs - getLastDshotBeaconCommandTimeUs() <
        ...
        if (!flipOverAfterCrashActive &&
            (canUseLaunchControl() ||
             (tryingToArm == ARMING_DELAYED_LAUNCH_CONTROL))) {
            if (launchControlState ==
                LAUNCH_CONTROL_DISABLED) {  // only activate if it hasn't
        ...
        disarmAt = currentTimeUs + armingConfig()->auto_disarm_delay *
                                     1e6;  // start disarm timeout, will be
                                           // extended when throttle is nonzero
...
static void updateInflightCalibrationState(void) {
    if (AccInflightCalibrationArmed && ARMING_FLAG(ARMED) &&
        ...
        !IS_RC_MODE_ACTIVE(BOXARM)) {  // Copter is airborne and you are turning
                                       // it off via boxarm : start measurement
        InflightcalibratingA = 50;
        AccInflightCalibrationArmed = false;
    }
    if (IS_RC_MODE_ACTIVE(BOXCALIB)) {  // Use the Calib Option to activate :
                                        // Calib = TRUE measurement started,
                                        // Land and Calib = 0 measurement stored
...
void updateMagHold(void) {
    if (fabsf(rcCommand[YAW]) < 15 && FLIGHT_MODE(MAG_MODE)) {
        ...
        if (STATE(SMALL_ANGLE)) {
            rcCommand[YAW] -=
              dif * currentPidProfile->pid[PID_MAG].P / 30;  // 18 deg
        }
    } else
...
// determine if the R/P/Y stick deflection exceeds the specified limit - integer
// math is good enough here.
bool areSticksActive(uint8_t stickPercentLimit) {
    for (int axis = FD_ROLL; axis <= FD_YAW; axis++) {
        const uint8_t deadband = axis == FD_YAW
                                   ? rcControlsConfig()->yaw_deadband
                                   : rcControlsConfig()->deadband;
        uint8_t stickPercent = 0;
        if ((rcData[axis] >= PWM_RANGE_MAX) ||
            (rcData[axis] <= PWM_RANGE_MIN)) {
            stickPercent = 100;
        } else {
            if (rcData[axis] > (rxConfig()->midrc + deadband)) {
                stickPercent =
                  ((rcData[axis] - rxConfig()->midrc - deadband) * 100) /
                  (PWM_RANGE_MAX - rxConfig()->midrc - deadband);
            } else if (rcData[axis] < (rxConfig()->midrc - deadband)) {
                stickPercent =
                  ((rxConfig()->midrc - deadband - rcData[axis]) * 100) /
                  (rxConfig()->midrc - deadband - PWM_RANGE_MIN);
            }
        }
...
int8_t calculateThrottlePercent(void) {
    ...
    if (featureIsEnabled(FEATURE_3D) && !IS_RC_MODE_ACTIVE(BOX3D) &&
        !flight3DConfig()->switched_mode3d) {
        if (channelData >
            (rxConfig()->midrc + flight3DConfig()->deadband3d_throttle)) {
            ret = ((channelData - rxConfig()->midrc -
                    flight3DConfig()->deadband3d_throttle) *
                   100) /
                  (PWM_RANGE_MAX - rxConfig()->midrc -
                   flight3DConfig()->deadband3d_throttle);
        } else if (channelData < (rxConfig()->midrc -
                                  flight3DConfig()->deadband3d_throttle)) {
            ret = -((rxConfig()->midrc - flight3DConfig()->deadband3d_throttle -
                     channelData) *
                    100) /
                  (rxConfig()->midrc - flight3DConfig()->deadband3d_throttle -
                   PWM_RANGE_MIN);
        ...
        ret = constrain(((channelData - rxConfig()->mincheck) * 100) /
...
bool processRx(timeUs_t currentTimeUs) {
    ...
    /* In airmode iterm should be prevented to grow when Low thottle and Roll +
     Pitch Centered. This is needed to prevent iterm winding on the ground, but
     keep full stabilisation on 0 throttle while in air */
...
#ifdef USE_RUNAWAY_TAKEOFF
    // If runaway_takeoff_prevention is enabled, accumulate the amount of time
    // that throttle is above runaway_takeoff_deactivate_throttle with the any
    // of the R/P/Y sticks deflected to at least runaway_takeoff_stick_percent
    // percent while the pidSum on all axis is kept low. Once the amount of
    // accumulated time exceeds runaway_takeoff_deactivate_delay then disable
    // prevention for the remainder of the battery.

    if (ARMING_FLAG(ARMED) && pidConfig()->runaway_takeoff_prevention &&
        !runawayTakeoffCheckDisabled && !flipOverAfterCrashActive &&
        !runawayTakeoffTemporarilyDisabled && !STATE(FIXED_WING)) {
        // Determine if we're in "flight"
        //   - motors running
        //   - throttle over runaway_takeoff_deactivate_throttle_percent
        //   - sticks are active and have deflection greater than
        //   runaway_takeoff_deactivate_stick_percent
        //   - pidSum on all axis is less then
        //   runaway_takeoff_deactivate_pidlimit
        bool inStableFlight = false;
        if (!featureIsEnabled(FEATURE_MOTOR_STOP) || airmodeIsEnabled() ||
            (throttleStatus != THROTTLE_LOW)) {  // are motors running?
            const uint8_t lowThrottleLimit =
              pidConfig()->runaway_takeoff_deactivate_throttle;
            const uint8_t midThrottleLimit =
              constrain(lowThrottleLimit * 2,
                        lowThrottleLimit * 2,
                        RUNAWAY_TAKEOFF_HIGH_THROTTLE_PERCENT);
            if ((((throttlePercent >= lowThrottleLimit) &&
                  areSticksActive(RUNAWAY_TAKEOFF_DEACTIVATE_STICK_PERCENT)) ||
                 (throttlePercent >= midThrottleLimit)) &&
                (fabsf(pidData[FD_PITCH].Sum) <
                 RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT) &&
                (fabsf(pidData[FD_ROLL].Sum) <
                 RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT) &&
                (fabsf(pidData[FD_YAW].Sum) <
                 RUNAWAY_TAKEOFF_DEACTIVATE_PIDSUM_LIMIT)) {
                inStableFlight = true;
        ...
        // If we're in flight, then accumulate the time and deactivate once it
        // exceeds runaway_takeoff_deactivate_delay milliseconds
        if (inStableFlight) {
            if (runawayTakeoffDeactivateUs == 0) {
                runawayTakeoffDeactivateUs = currentTimeUs;
            }
            uint16_t deactivateDelay =
            ...
            if ((cmpTimeUs(currentTimeUs, runawayTakeoffDeactivateUs) +
                 runawayTakeoffAccumulatedUs) > deactivateDelay * 1000) {
            ...
            if (runawayTakeoffDeactivateUs != 0) {
                runawayTakeoffAccumulatedUs +=
                  cmpTimeUs(currentTimeUs, runawayTakeoffDeactivateUs);
            }
            runawayTakeoffDeactivateUs = 0;
        }
        if (runawayTakeoffDeactivateUs == 0) {
            ...
            DEBUG_SET(DEBUG_RUNAWAY_TAKEOFF,
                      ...
                      runawayTakeoffAccumulatedUs / 1000);
        } else {
            DEBUG_SET(DEBUG_RUNAWAY_TAKEOFF,
                      DEBUG_RUNAWAY_TAKEOFF_DEACTIVATING_DELAY,
    ...
    if (ARMING_FLAG(ARMED)) {
        if (launchControlActive &&
            (throttlePercent >
             currentPidProfile->launchControlThrottlePercent)) {
            // throttle limit trigger reached, launch triggered
            // reset the iterms as they may be at high values from holding the
            // launch position
        ...
        if (launchControlState == LAUNCH_CONTROL_TRIGGERED) {
            ...
            // For trigger mode SINGLE we never reset the state and only a
            // single launch is allowed until a reboot.
            if (currentPidProfile->launchControlAllowTriggerReset &&
                !IS_RC_MODE_ACTIVE(BOXLAUNCHCONTROL)) {
    ...
    if (ARMING_FLAG(ARMED) && featureIsEnabled(FEATURE_MOTOR_STOP) &&
        !STATE(FIXED_WING) && !featureIsEnabled(FEATURE_3D) &&
        !airmodeIsEnabled() &&
        !FLIGHT_MODE(
          GPS_RESCUE_MODE)  // disable auto-disarm when GPS Rescue is active
    ) {
        if (isUsingSticksForArming()) {
        ...
        disarmAt =
          currentTimeUs + autoDisarmDelayUs;  // extend auto-disarm timer
    }
    ...
    if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE)) {
        LED1_ON;
        // increase frequency of attitude task to reduce drift when in angle or
        // horizon mode
    ...
    if (featureIsEnabled(FEATURE_TELEMETRY)) {
        bool enableSharedPortTelemetry =
        ...
        if (enableSharedPortTelemetry && !sharedPortTelemetryEnabled) {
            ...
            // the telemetry state must be checked immediately so that shared
            // serial ports are released.
            telemetryCheckState();
...
#ifdef USE_ACRO_TRAINER
    pidSetAcroTrainerState(IS_RC_MODE_ACTIVE(BOXACROTRAINER) &&
                           sensors(SENSOR_ACC));
#endif  // USE_ACRO_TRAINER
    ...
    pidSetAntiGravityState(IS_RC_MODE_ACTIVE(BOXANTIGRAVITY) ||
...
#ifdef USE_RUNAWAY_TAKEOFF
    // Check to see if runaway takeoff detection is active (anti-taz), the
    // pidSum is over the threshold, and gyro rate for any axis is above the
    // limit for at least the activate delay period. If so, disarm for safety
    if (ARMING_FLAG(ARMED) && !STATE(FIXED_WING) &&
        pidConfig()->runaway_takeoff_prevention &&
        !runawayTakeoffCheckDisabled && !flipOverAfterCrashActive &&
        !runawayTakeoffTemporarilyDisabled &&
        (!featureIsEnabled(FEATURE_MOTOR_STOP) || airmodeIsEnabled() ||
         (calculateThrottleStatus() != THROTTLE_LOW))) {
        if (((fabsf(pidData[FD_PITCH].Sum) >=
            ...
            if (runawayTakeoffTriggerUs == 0) {
                runawayTakeoffTriggerUs =
                  currentTimeUs + RUNAWAY_TAKEOFF_ACTIVATE_DELAY;
        ...
        DEBUG_SET(DEBUG_RUNAWAY_TAKEOFF,
                  ...
                  runawayTakeoffTriggerUs == 0 ? DEBUG_RUNAWAY_TAKEOFF_FALSE
                                               : DEBUG_RUNAWAY_TAKEOFF_TRUE);
    } else {
        runawayTakeoffTriggerUs = 0;
        DEBUG_SET(DEBUG_RUNAWAY_TAKEOFF,
...
#ifdef USE_SERVOS
    // motor outputs are used as sources for servo mixing, so motors must be
    // calculated using mixTable() before servos.
