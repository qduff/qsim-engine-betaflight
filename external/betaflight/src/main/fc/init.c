...
void init(void) {
    ...
    // Opportunistically use the first motor pin of the default configuration
    // for detection. We are doing this as with some boards, timing seems to be
    // important, and the later detection will fail.
    ...
    if (!readSuccess || !isEEPROMVersionValid() ||
        strncasecmp(systemConfig()->boardIdentifier,
...
#ifdef USE_BRUSHED_ESC_AUTODETECT
    // Now detect again with the actually configured pin for motor 1, if it is
    // not the default pin.
    ...
    delayMicroseconds(
      10);  // allow configuration to settle // XXX Could be removed, too?

    // Allow EEPROM reset with two-button-press without power cycling in DEBUG
    // build
#ifdef DEBUG
...
#undef EEPROM_RESET_PRECONDITION
    ...
    if (featureIsEnabled(FEATURE_RX_SERIAL)) {
        switch (rxConfig()->serialrx_provider) {
            ...
            case SERIALRX_SRXL:
                ...
                // Must be called before that 100ms sleep so that we don't lose
                // satellite's binding window after startup. The rest of
                // Spektrum initialization will happen later - via
                // spektrumInit()
    ...
    /* Motors needs to be initialized soon as posible because hardware
     * initialization may send spurious pulses to esc's causing their early
     * initialization. Also ppm receiver may share timer with motors so motors
...
#if defined(STM32H7) && \
  defined(USE_SDCARD_SDIO)  // H7 only for now, likely should be applied to
                            // F4/F7 too
...
// XXX And these no longer work properly as FEATURE_RANGEFINDER does control
// HCSR04 runtime configuration.
#if defined(RANGEFINDER_HCSR04_SOFTSERIAL2_EXCLUSIVE) && \
  defined(USE_RANGEFINDER_HCSR04) && defined(USE_SOFTSERIAL2)
    if (featureIsEnabled(FEATURE_RANGEFINDER) &&
        featureIsEnabled(FEATURE_SOFTSERIAL)) {
        serialRemovePort(SERIAL_PORT_SOFTSERIAL2);
    }
#endif

#if defined(RANGEFINDER_HCSR04_SOFTSERIAL1_EXCLUSIVE) && \
  defined(USE_RANGEFINDER_HCSR04) && defined(USE_SOFTSERIAL1)
    if (featureIsEnabled(FEATURE_RANGEFINDER) &&
        featureIsEnabled(FEATURE_SOFTSERIAL)) {
    ...
    if (!sensorsAutodetect()) {
        // if gyro was not detected due to whatever reason, notify and don't
        // arm.
    ...
    // so we are ready to call validateAndFixGyroConfig(), pidInit(), and
    ...
    if (isMixerUsingServos()) {
        // pwm_params.useChannelForwarding =
...
#if defined(USE_OSD)
    // The OSD need to be initialised after GYRO to avoid GYRO initialisation
...
#elif defined(USE_CMS) && defined(USE_MSP_DISPLAYPORT) && \
  defined(USE_OSD_OVER_MSP_DISPLAYPORT)  // OSD over MSP; not supported (yet)
    ...
    if (!osdDisplayPort) cmsDisplayPortRegister(displayPortMspInit());
#endif
...
#ifdef USE_VTX_RTC6705
    if (!vtxCommonDevice() &&
        useRTC6705) {  // external VTX takes precedence when configured.
        vtxRTC6705Init();
    }
#endif
...
#ifdef SOFTSERIAL_LOOPBACK
    // FIXME this is a hack, perhaps add a FUNCTION_LOOPBACK to support it
    // properly
