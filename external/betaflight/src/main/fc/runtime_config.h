...
// FIXME some of these are flight modes, some of these are general status
// indicators
typedef enum {
    ARMED = (1 << 0),
    WAS_EVER_ARMED = (1 << 1),
...
typedef enum {
    ...
    ARMING_DISABLED_ARM_SWITCH =
      (1 << 21),  // Needs to be the last element, since it's always activated
                  // if one of the others is active when arming
...
// macro to initialize map from boxId_e to log2(flightModeFlags). Keep it in
// sync with flightModeFlags_e enum. [BOXARM] is left unpopulated
#define BOXID_TO_FLIGHT_MODE_MAP_INITIALIZER                              \
    {                                                                     \
        [BOXANGLE] = LOG2(ANGLE_MODE), [BOXHORIZON] = LOG2(HORIZON_MODE), \
        [BOXMAG] = LOG2(MAG_MODE), [BOXHEADFREE] = LOG2(HEADFREE_MODE),   \
...
typedef enum {
    ...
    CALIBRATE_MAG = (1 << 2),
    SMALL_ANGLE = (1 << 3),
    FIXED_WING = (1 << 4)  // set when in flying_wing or airplane mode.
                           // currently used by althold selection code
} stateFlags_t;
