...
bool cmsDisplayPortRegister(displayPort_t *pDisplay) {
    if (cmsDeviceCount == CMS_MAX_DEVICE) return false;
...
static displayPort_t *cmsDisplayPortSelectCurrent(void) {
    if (cmsDeviceCount == 0) return NULL;

    if (cmsCurrentDevice < 0) cmsCurrentDevice = 0;
...
#define CMS_POLL_INTERVAL_US \
    100000  // Interval of polling dynamic values (microsec)
...
static OSD_Entry menuErrEntries[] = {{"BROKEN MENU", OME_Label, NULL, NULL, 0},
...
static void cmsFormatFloat(int32_t value, char *floatString) {
    ...
    // oraz zero wiodonce
    if (floatString[0] == '0') floatString[0] = ' ';
}
...
static int cmsDrawMenuItemValue(displayPort_t *pDisplay,
                                char *buff,
                                uint8_t row,
                                uint8_t maxSize) {
    int colpos;
...
static int cmsDrawMenuEntry(displayPort_t *pDisplay,
                            const OSD_Entry *p,
                            uint8_t row,
                            bool selectedRow,
                            uint8_t *flags) {
    ...
    switch (p->type) {
        case OME_String:
            if (IS_PRINTVALUE(*flags) && p->data) {
                ...
                cnt = cmsDrawMenuItemValue(
                  pDisplay, buff, row, CMS_DRAW_BUFFER_LEN);
        ...
        case OME_Funcall:
            if (IS_PRINTVALUE(*flags)) {
                ...
                if ((p->type == OME_Submenu) && p->func &&
                    (*flags & OPTSTRING)) {
                    // Special case of sub menu entry with optional value
                    // display.
        ...
        case OME_UINT8:
            if (IS_PRINTVALUE(*flags) && p->data) {
                ...
                cnt =
                  cmsDrawMenuItemValue(pDisplay, buff, row, CMS_NUM_FIELD_LEN);
                CLR_PRINTVALUE(*flags);
            }
            break;
            ...
            if (IS_PRINTVALUE(*flags) && p->data) {
                ...
                cnt =
                  displayWrite(pDisplay,
                               leftMenuColumn + 1 + (uint8_t)strlen(p->text),
                               row,
                               p->data);
                CLR_PRINTVALUE(*flags);
            }
...
static void cmsDrawMenu(displayPort_t *pDisplay, uint32_t currentTimeUs) {
    ...
    if (pDisplay->cleared) {
        ...
        for (p = pageTop, i = 0; (p <= pageTop + pageMaxRow); p++, i++) {
            if (IS_DYNAMIC(p)) SET_PRINTVALUE(runtimeEntryFlags[i]);
        }
    }
    ...
    if (pDisplay->cursorRow >= 0 &&
        ...
        room -= displayWrite(pDisplay,
                             leftMenuColumn,
                             top + pDisplay->cursorRow * linesPerMenuItem,
                             " ");
    }
    ...
    if (pDisplay->cursorRow != currentCtx.cursorRow) {
        room -= displayWrite(pDisplay,
                             leftMenuColumn,
                             top + currentCtx.cursorRow * linesPerMenuItem,
                             ">");
    ...
    for (i = 0, p = pageTop; (p <= pageTop + pageMaxRow); i++, p++) {
        if (IS_PRINTLABEL(runtimeEntryFlags[i])) {
            ...
            room -= displayWrite(
              pDisplay, coloff, top + i * linesPerMenuItem, p->text);
        ...
        if (IS_PRINTVALUE(runtimeEntryFlags[i])) {
            ...
            room -= cmsDrawMenuEntry(pDisplay,
                                     p,
                                     top + i * linesPerMenuItem,
                                     selectedRow,
                                     &runtimeEntryFlags[i]);
            if (room < 30) return;
        }
...
STATIC_UNIT_TESTED long cmsMenuBack(
  displayPort_t *pDisplay);  // Forward; will be resolved after merging
...
long cmsMenuChange(displayPort_t *pDisplay, const void *ptr) {
    ...
    if (pMenu != currentCtx.menu) {
        ...
        // The (pMenu == curretMenu) case occurs when reopening for display
        // cycling currentCtx.cursorRow has been saved as absolute; convert it
...
STATIC_UNIT_TESTED long cmsMenuBack(displayPort_t *pDisplay) {
    ...
    if (currentCtx.menu->onExit &&
        currentCtx.menu->onExit(pageTop + currentCtx.cursorRow) < 0) {
        return -1;
    }
...
long cmsMenuExit(displayPort_t *pDisplay, const void *ptr) {
    ...
    switch (exitType) {
        ...
        case CMS_POPUP_SAVEREBOOT:
            ...
            if ((exitType == CMS_POPUP_SAVE) ||
                ...
                // traverse through the menu stack and call their onExit
                // functions
    ...
    if ((exitType == CMS_EXIT_SAVEREBOOT) ||
        ...
        displayResync(
          pDisplay);  // Was max7456RefreshAll(); why at this timing?
...
STATIC_UNIT_TESTED uint16_t cmsHandleKey(displayPort_t *pDisplay,
                                         cms_key_e key) {
    uint16_t res = BUTTON_TIME;
    const OSD_Entry *p;

    if (!currentCtx.menu) return res;
    ...
    if ((key == CMS_KEY_UP) && (!osdElementEditing)) {
        ...
        if ((pageTop + currentCtx.cursorRow)->type == OME_Label &&
            currentCtx.cursorRow > 0)
            currentCtx.cursorRow--;

        if (currentCtx.cursorRow == -1 ||
            (pageTop + currentCtx.cursorRow)->type == OME_Label) {
            // Goto previous page
    ...
    switch (p->type) {
        ...
        case OME_Funcall:;
            ...
            if (p->func && key == CMS_KEY_RIGHT) {
                ...
                if (retval == MENU_CHAIN_BACK) cmsMenuBack(pDisplay);
                res = BUTTON_PAUSE;
            }
            break;
        ...
        case OME_FLOAT:
            if (p->data) {
                OSD_UINT8_t *ptr = p->data;
                if (key == CMS_KEY_RIGHT) {
                    if (*ptr->val < ptr->max) *ptr->val += ptr->step;
                } else {
                    if (*ptr->val > ptr->min) *ptr->val -= ptr->step;
                }
                SET_PRINTVALUE(runtimeEntryFlags[currentCtx.cursorRow]);
                if (p->func) {
                    p->func(pDisplay, p);
        ...
        case OME_TAB:
            if (p->type == OME_TAB) {
                ...
                if (key == CMS_KEY_RIGHT) {
                    if (*ptr->val < ptr->max) *ptr->val += 1;
                } else {
                    if (*ptr->val > 0) *ptr->val -= 1;
                }
                if (p->func) p->func(pDisplay, p->data);
                SET_PRINTVALUE(runtimeEntryFlags[currentCtx.cursorRow]);
            }
            break;
...
void cmsSetExternKey(cms_key_e extKey) {
    if (externKey == CMS_KEY_NONE) externKey = extKey;
}

uint16_t cmsHandleKeyWithRepeat(displayPort_t *pDisplay,
                                cms_key_e key,
                                int repeatCount) {
    uint16_t ret = 0;
...
static void cmsUpdate(uint32_t currentTimeUs) {
    ...
    if (!cmsInMenu) {
        ...
        if (IS_MID(THROTTLE) && IS_LO(YAW) && IS_HI(PITCH) &&
            !ARMING_FLAG(ARMED)) {
            cmsMenuOpen();
        ...
        if (externKey != CMS_KEY_NONE) {
            ...
            if (rcDelayMs > 0) {
                ...
                rcDelayMs =
                  cmsHandleKeyWithRepeat(pCurrentDisplay, key, repeatCount);

                // Key repeat effect is implemented in two phases.
                // First phldase is to decrease rcDelayMs reciprocal to hold
                // time. When rcDelayMs reached a certain limit (scheduling
                // interval), repeat rate will not raise anymore, so we call key
                // handler multiple times (repeatCount).
                //
                ...
                if (((key == CMS_KEY_LEFT) || (key == CMS_KEY_RIGHT)) &&
                    (holdCount > 20)) {
                    ...
                    if (rcDelayMs <= 50) {
                        ...
                        if (repeatBase == 0) repeatBase = holdCount;

                        repeatCount =
                          repeatCount + (holdCount - repeatBase) / 5;
