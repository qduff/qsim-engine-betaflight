...
#define ATTITUDE_RESET_QUIET_TIME \
    250000  // 250ms - gyro quiet period after disarm before attitude reset
#define ATTITUDE_RESET_GYRO_LIMIT \
    15  // 15 deg/sec - gyro limit for quiet period
#define ATTITUDE_RESET_KP_GAIN \
    25.0  // dcmKpGain value to use during attitude reset
#define ATTITUDE_RESET_ACTIVE_TIME \
    500000  // 500ms - Time to wait for attitude to converge at high gain
#define GPS_COG_MIN_GROUNDSPEED \
    500  // 500cm/s minimum groundspeed for a gps heading to be considered valid
...
void imuConfigure(uint16_t throttle_correction_angle,
    ...
    fc_acc =
      calculateAccZLowPassFilterRCTimeConstant(5.0f);  // Set to fix value
...
static void imuMahonyAHRSupdate(float dt,
    ...
    if (useCOG) {
        ...
        const float ez_ef = (-sin_approx(courseOverGround) * rMat[0][0] -
    ...
    if (useMag && recipMagNorm > 0.01f) {
        ...
        // For magnetometer correction we make an assumption that magnetic field
        // is perpendicular to gravity (ignore Z-component in EF). This way
        // magnetic field will only affect heading and wont mess roll/pitch
        // angles

        // (hx; hy; 0) - measured mag field vector in EF (assuming Z-component
        // is zero) (bx; 0; 0) - reference mag field vector heading due North in
        // EF (assuming Z-component is zero)
        const float hx = rMat[0][0] * mx + rMat[0][1] * my + rMat[0][2] * mz;
        const float hy = rMat[1][0] * mx + rMat[1][1] * my + rMat[1][2] * mz;
        const float bx = sqrtf(hx * hx + hy * hy);

        // magnetometer error is cross product between estimated magnetic north
        // and measured magnetic north (calculated in EF)
...
STATIC_UNIT_TESTED void imuUpdateEulerAngles(void) {
    ...
    if (FLIGHT_MODE(HEADFREE_MODE)) {
        ...
        attitude.values.roll =
          lrintf(atan2_approx((+2.0f * (buffer.wx + buffer.yz)),
                              (+1.0f - 2.0f * (buffer.xx + buffer.yy))) *
                 (1800.0f / M_PIf));
        attitude.values.pitch = lrintf(
          ((0.5f * M_PIf) - acos_approx(+2.0f * (buffer.wy - buffer.xz))) *
          (1800.0f / M_PIf));
        attitude.values.yaw =
          lrintf((-atan2_approx((+2.0f * (buffer.wz + buffer.xy)),
                                (+1.0f - 2.0f * (buffer.yy + buffer.zz))) *
                  (1800.0f / M_PIf)));
    } else {
        attitude.values.roll =
          lrintf(atan2_approx(rMat[2][1], rMat[2][2]) * (1800.0f / M_PIf));
        attitude.values.pitch = lrintf(
          ((0.5f * M_PIf) - acos_approx(-rMat[2][0])) * (1800.0f / M_PIf));
        attitude.values.yaw =
          lrintf((-atan2_approx(rMat[1][0], rMat[0][0]) * (1800.0f / M_PIf)));
    }
...
// imuRuntimeConfig.dcm_kp * 1.0 scaling. When disarmed after initial boot, the
// scaling is set to 10.0 for the first 20 seconds to speed up initial
// convergence. After disarming we want to quickly reestablish convergence to
// deal with the attitude estimation being incorrect due to a crash.
//   - wait for a 250ms period of low gyro activity to ensure the craft is not
//   moving
//   - use a large dcmKpGain value for 500ms to allow the attitude estimate to
//   quickly converge
//   - reset the gain back to the standard setting
static float imuCalcKpGain(timeUs_t currentTimeUs,
                           bool useAcc,
                           float *gyroAverage) {
    ...
    if (!armState) {
        ...
        // Also, if the attitude reset has been complete and there is subsequent
        // gyro activity then start the reset cycle again. This addresses the
        // case where the pilot rights the craft after a crash.
        ...
        if (attitudeResetTimeEnd > 0) {  // Resetting the attitude estimation
            ...
            // Start the high gain period to bring the estimation into
            // convergence
    ...
    if (attitudeResetActive) {
        ...
        if (!armState) {
            ret = ret * 10.0f;  // Scale the kP to generally converge faster
                                // when disarmed.
        }
    }
...
static void imuCalculateEstimatedAttitude(timeUs_t currentTimeUs) {
    ...
    bool useAcc = false;
    bool useMag = false;
    bool useCOG = false;  // Whether or not correct yaw via imuMahonyAHRSupdate
                          // from our ground course
    float courseOverGround =
...
#endif
#if defined(USE_GPS)
    if (!useMag && sensors(SENSOR_GPS) && STATE(GPS_FIX) &&
        gpsSol.numSat >= 5 && gpsSol.groundSpeed >= GPS_COG_MIN_GROUNDSPEED) {
        ...
        if (useCOG && shouldInitializeGPSHeading()) {
            // Reset our reference and reinitialize quaternion.  This will
            // likely ideally happen more than once per flight, but for now,
            // shouldInitializeGPSHeading() returns true only once.
            imuComputeQuaternionFromRPY(&qP,
                                        attitude.values.roll,
                                        attitude.values.pitch,
                                        gpsSol.groundCourse);

            useCOG = false;  // Don't use the COG when we first reinitialize.
                             // Next time around though, yes.
        }
    }
#endif
    ...
    //  printf("[imu]deltaT = %u, imuDeltaT = %u, currentTimeUs = %u,
    //  micros64_real = %lu\n", deltaT, imuDeltaT, currentTimeUs,
    //  micros64_real());
    deltaT = imuDeltaT;
#endif
    ...
    imuMahonyAHRSupdate(deltaT * 1e-6f,
                        DEGREES_TO_RADIANS(gyroAverage[X]),
                        DEGREES_TO_RADIANS(gyroAverage[Y]),
                        DEGREES_TO_RADIANS(gyroAverage[Z]),
                        useAcc,
                        accAverage[X],
                        accAverage[Y],
                        accAverage[Z],
...
static int calculateThrottleAngleCorrection(void) {
    /*
     * Use 0 as the throttle angle correction if we are inverted, vertical or
     * with a small angle < 0.86 deg
    ...
    return lrintf(throttleAngleValue *
                  sin_approx(angle / (900.0f * M_PIf / 2.0f)));
}
...
void imuComputeQuaternionFromRPY(quaternionProducts *quatProd,
                                 int16_t initialRoll,
    ...
    const float cosRoll =
      cos_approx(DECIDEGREES_TO_RADIANS(initialRoll) * 0.5f);
    const float sinRoll =
      sin_approx(DECIDEGREES_TO_RADIANS(initialRoll) * 0.5f);

    const float cosPitch =
      cos_approx(DECIDEGREES_TO_RADIANS(initialPitch) * 0.5f);
    const float sinPitch =
      sin_approx(DECIDEGREES_TO_RADIANS(initialPitch) * 0.5f);
...
bool imuQuaternionHeadfreeOffsetSet(void) {
    if ((ABS(attitude.values.roll) < 450) &&
        (ABS(attitude.values.pitch) < 450)) {
        const float yaw = -atan2_approx((+2.0f * (qP.wz + qP.xy)),
                                        (+1.0f - 2.0f * (qP.yy + qP.zz)));
...
void imuQuaternionMultiplication(quaternion *q1,
                                 quaternion *q2,
                                 quaternion *result) {
...
void imuQuaternionHeadfreeTransformVectorEarthToBody(t_fp_vector_def *v) {
    ...
    const float x = (buffer.ww + buffer.xx - buffer.yy - buffer.zz) * v->X +
                    2.0f * (buffer.xy + buffer.wz) * v->Y +
                    2.0f * (buffer.xz - buffer.wy) * v->Z;
    const float y = 2.0f * (buffer.xy - buffer.wz) * v->X +
                    (buffer.ww - buffer.xx + buffer.yy - buffer.zz) * v->Y +
                    2.0f * (buffer.yz + buffer.wx) * v->Z;
    const float z = 2.0f * (buffer.xz + buffer.wy) * v->X +
                    2.0f * (buffer.yz - buffer.wx) * v->Y +
