...
void timerInit(void) {
    printf("[DRIVER/EEPROM] TimerInit...\n");
}
...
void indicateFailure(failureMode_e mode, int repeatCount) {
    UNUSED(repeatCount);
    printf("[DRIVER] Failure LED flash for: [failureMode]!!! %d\n", mode);
}
...
static char *EEPROM_FILENAME = "eeprom.bin";

void FLASH_Unlock(void) {
    if (eepromFd != NULL) {
        fprintf(stderr, "[DRIVER/EEPROM] [FLASH_Unlock] eepromFd != NULL\n");
        return;
    }
    ...
    if (eepromFd != NULL) {
        ...
        if (n == lSize) {
            printf(
              "[DRIVER/EEPROM] [FLASH_Unlock] loaded '%s', size = %ld / %ld\n",
              EEPROM_FILENAME,
              lSize,
              sizeof(eepromData));
        } else {
            fprintf(stderr,
                    "[DRIVER/EEPROM]  [FLASH_Unlock] failed to load '%s, %ld < "
                    "%ld'\n",
                    EEPROM_FILENAME,
                    n,
                    lSize);
        ...
        printf("[DRIVER/EEPROM] [FLASH_Unlock] created '%s', size = %ld\n",
               EEPROM_FILENAME,
               sizeof(eepromData));
        if ((eepromFd = fopen(EEPROM_FILENAME, "wb+")) == NULL) {
            fprintf(stderr,
                    "[DRIVER/EEPROM] [FLASH_Unlock] failed to create '%s'\n",
                    EEPROM_FILENAME);
            return;
        }
        if (fwrite(eepromData, sizeof(eepromData), 1, eepromFd) != 1) {
            fprintf(stderr,
                    "[DRIVER/EEPROM] [FLASH_Unlock] write failed: %s\n",
                    strerror(errno));
        }
    }
}
...
void FLASH_Lock(void) {
    ...
    if (eepromFd != NULL) {
        ...
        fclose(eepromFd);
        eepromFd = NULL;
        printf("[DRIVER/EEPROM] [FLASH_Lock] saved '%s size: %d'\n",
               EEPROM_FILENAME,
               wrote);
    } else {
        fprintf(stderr,
                "[DRIVER/EEPROM] [FLASH_Lock] eeprom is not unlocked\n");
    }
}
...
void pgResetFn_serialPinConfig(serialPinConfig_t *serialPinConfig) {
    printf("[DRIVER/EEPROM] Serial reset\n");
}
...
uint32_t persistentObjectRead(int id) {
    UNUSED(id);
    printf("[DRIVER/EEPROM] persistent read\n");
}

void persistentObjectWrite(int id, uint32_t value) {
    UNUSED(id);
    UNUSED(value);
    printf("[DRIVER/EEPROM] persistent write\n");
}
